...
static void
test_random_base64_encoding(base64_character_set const& character_set)
{
    for (int i = 0; i != 100; ++i)
    {
...
TEST_CASE("base64_encoding_test")
{
    check_mime_encoding(
    ...
    check_mime_encoding("leasure.", "bGVhc3VyZS4=");

    check_mime_encoding("easure.", "ZWFzdXJlLg==");

    check_mime_encoding("asure.", "YXN1cmUu");

    check_mime_encoding("sure.", "c3VyZS4=");

    check_mime_encoding("sure", "c3VyZQ==");

    test_random_base64_encoding(get_url_friendly_base64_character_set());
    test_random_base64_encoding(get_mime_base64_character_set());
}

static void
check_base36_round_trip(
...
static void
check_invalid_base36(string const& text)
{
    REQUIRE_THROWS_AS(base36_decode(text), exception);
}

TEST_CASE("base36_encoding_test")
{
...
static void
check_nonsequential_base36_round_trip(
    uint64_t id, unsigned minimum_length, string const& correct_encoding)
{
...
static void
check_nonsequential_base36_series(uint64_t n_ids, unsigned minimum_length)
{
    std::set<string> ids;
    ...
    REQUIRE(ids.size() == n_ids);
}

TEST_CASE("nonsequential_base36_encoding_test")
{
