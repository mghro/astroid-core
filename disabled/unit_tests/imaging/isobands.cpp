...
{
    double area = 0;
    for (std::vector<triangle<2, double>>::const_iterator i = tris.begin();
         i != tris.end();
         ++i)
    {
        area += get_area(*i);
    ...
    std::vector<triangle<2, double>> const& tris)
{
    for (std::vector<line_segment<2, double>>::const_iterator i
         ...
         ++i)
    {
        for (std::vector<triangle<2, double>>::const_iterator j = tris.begin();
             j != tris.end();
             ++j)
        {
            for (int k = 0; k != 2; ++k)
...
std::vector<polygon2>
as_polygons(std::vector<triangle<2, double>> const& tris)
{
...
bool
all_ccw(std::vector<triangle<2, double>> const& tris)
{
    for (std::vector<triangle<2, double>>::const_iterator i = tris.begin();
         i != tris.end();
         ++i)
...
TEST_CASE("isobands_test")
{
    ...
    REQUIRE(all_ccw(low_tris));
    REQUIRE(all_ccw(middle_tris));
    REQUIRE(all_ccw(high_tris));
    ...
    polyset overlap;
    do_set_operation(
        &overlap, set_operation::INTERSECTION, low_region, middle_region);
    REQUIRE(almost_equal(get_area(overlap), 0., tolerance));
    do_set_operation(
        &overlap, set_operation::INTERSECTION, low_region, high_region);
    REQUIRE(almost_equal(get_area(overlap), 0., tolerance));
    do_set_operation(
        &overlap, set_operation::INTERSECTION, middle_region, high_region);
    REQUIRE(almost_equal(get_area(overlap), 0., tolerance));
    ...
    polyset full_region;
    do_set_operation(
        &full_region, set_operation::UNION, low_region, middle_region);
    do_set_operation(
        &full_region, set_operation::UNION, full_region, high_region);
    polyset img_box;
    create_polyset(&img_box, as_polygon(get_bounding_box(img)));
    REQUIRE(almost_equal(full_region, img_box, 0.001));
    ...
    do_set_operation(
        &overlap, set_operation::INTERSECTION, inner_region, middle_region);
    REQUIRE(almost_equal(overlap, inner_region, tolerance));
    ...
    for (unsigned i = 0; i != 54; ++i)
    {
        for (unsigned j = 0; j != 18; ++j)
        {
            ...
            double value = apply(
                img.value_mapping, get_pixel_ref(img, make_vector(j, i)));
            polyset* region;
            if (value < 2)
                region = &low_region;
            else if (value < 4)
    ...
    REQUIRE(isolines_match_isobands(low_lines, low_tris));
    REQUIRE(isolines_match_isobands(low_lines, middle_tris));
    auto high_lines = compute_isolines(img, 4);
    REQUIRE(isolines_match_isobands(high_lines, middle_tris));
    close_isoline_contours(high_lines, img, 4);
    REQUIRE(isolines_match_isobands(high_lines, high_tris));
}
