...
struct composition_cache
{
    ...
    explicit composition_cache(DataContext const& context) : context(&context)
    {
    }
};
...
#define CRADLE_INVOKE_COMPOSER(generate_id, call_function)                    \
    ...
    {                                                                         \
        ...
        entry = cache.entries                                                 \
                    .insert(composition_cache_entry_map::value_type(          \
                        &new_entry.call_id.get(), new_entry))                 \
                    .first;                                                   \
...
auto
invoke_composer(Function const& f, composition_cache<DataContext>& cache)
{
    CRADLE_INVOKE_COMPOSER(make_id(f), f(cache))
}
// 1 arg
template<class DataContext, class Function, class Arg0>
auto
invoke_composer(
    Function const& f, composition_cache<DataContext>& cache, Arg0 const& arg0)
{
    CRADLE_INVOKE_COMPOSER(
        combine_ids(make_id(f), make_id_by_reference(arg0)), f(cache, arg0))
}
// 2 args
            ...
            combine_ids(
                combine_ids(
                    combine_ids(
                        combine_ids(
                            combine_ids(
                                make_id(f), make_id_by_reference(arg0)),
                            make_id_by_reference(arg1)),
...
template<class DataContext, class Function>
auto
invoke_leaf_composer(Function const& f, composition_cache<DataContext>& cache)
{
    CRADLE_INVOKE_COMPOSER(make_id(f), f(get_data_context(cache)))
}
// 1 arg
template<class DataContext, class Function, class Arg0>
auto
...
{
    composition_cache_wrapper(Function function) : function_(function)
    {
    }
