...
class fixed
{
    ...
    fixed(tag _, Integer value) : value_(value)
    {
    }
 ...
 public:
    ...
    fixed(fixed const& other) : value_(other.value_)
    {
    }

    explicit fixed(float x) : value_(Integer(x * float(1 << FractionalBits)))
    {
    }
    explicit fixed(double x) : value_(Integer(x * double(1 << FractionalBits)))
    {
    }
    explicit fixed(Integer x) : value_(x << FractionalBits)
    {
    }
    ...
    operator*(fixed other) const
    {
        return fixed(
            tag(),
            (LargerInteger(value_) * LargerInteger(other.value_))
                >> FractionalBits);
    }
    ...
    operator/(fixed other) const
    {
        return fixed(
            tag(),
            (LargerInteger(value_) << FractionalBits)
