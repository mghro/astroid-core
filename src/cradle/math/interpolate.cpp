...
compute_interpolation_grid(
    std::vector<double> const& sample_positions, double growth_tolerance)
{
    ...
    double const lower_edge
        = sample_positions[0]
          - (sample_positions[1] - sample_positions[0]) / 2;
    double const upper_edge
        = sample_positions.back()
    ...
    std::vector<double>::const_iterator i = sample_positions.begin(),
                                        next_i = i + 1;
    ...
    double best_spacing
        = (std::max)(average_spacing / growth_tolerance, smallest_spacing);
