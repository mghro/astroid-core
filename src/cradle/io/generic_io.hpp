...
value static inline parse_json_value(char const* json, size_t length)
{
    value v;
...
void static inline parse_json_value(value* v, string const& json)
{
    return parse_json_value(v, json.c_str(), json.length());
}

value static inline parse_json_value(string const& json)
{
    value v;
...
string static inline value_to_json(value const& v)
{
    string json;
...
void
deserialize_value(
    value* v, uint8_t const* data, size_t size, uint32_t* crc = 0);

void
serialize_value(byte_vector* data, value const& v, uint32_t* crc = 0);

// BASE-64 - CRC'd conversion to and from base-64 strings

void
parse_base64_value_string(value* v, string const& s, uint32_t* crc = 0);

value static inline parse_base64_value_string(string const& s)
{
    value v;
...
void
value_to_base64_string(string* s, value const& v, uint32_t* crc = 0);

string static inline value_to_base64_string(value const& v)
{
    string s;
...
void
read_value_file(value* v, file_path const& file, uint32_t* crc = 0);
void
write_value_file(file_path const& file, value const& v, uint32_t* crc = 0);

template<class T>
T
read_value_file_as(cradle::file_path const& file)
{
    value v;
