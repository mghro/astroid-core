...
{
    size_t
    operator()(cradle::file_path const& x) const
    {
        return cradle::invoke_hash(x.string<std::string>());
    }
};
...
struct file_error : exception
{
    file_error(file_path const& file, string const& msg)
        : exception(file.string() + ": " + msg), file_(new file_path(file))
    {
    }
    ~file_error() throw()
    {
 ...
 private:
    std::shared_ptr<file_path> file_;
};
...
{
    open_file_error(
        file_path const& file, string const& msg = "unable to open file")
        : file_error(file, msg)
    {
    }
...
struct c_file : noncopyable
{
    ...
    c_file(FILE* f, file_path const& path) : f_(f), path_(path)
    {
    }
...
struct simple_file_parser
{
    ...
    void
    throw_error(string const& msg) const;
...
inline simple_file_parser&
operator>>(simple_file_parser& p, int& i)
{
    i = p.read_int();
    return p;
}
inline simple_file_parser&
operator>>(simple_file_parser& p, unsigned& u)
{
    u = p.read_unsigned();
    return p;
}
inline simple_file_parser&
operator>>(simple_file_parser& p, float& f)
{
    f = float(p.read_double());
    return p;
}
inline simple_file_parser&
operator>>(simple_file_parser& p, double& d)
{
    d = p.read_double();
    return p;
}
