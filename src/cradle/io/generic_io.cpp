...
blob
string_to_blob(string const& s)
{
    blob blob;
    // Don't include the terminating '\0'.
    std::shared_ptr<char> ptr(new char[s.length()], array_deleter<char>());
    blob.ownership = ptr;
    blob.data = ptr.get();
...
bool static list_resembles_map(Json::Value const& json)
{
...
void static read_json_value(
    value* v, check_in_interface& check_in, Json::Value const& json)
{
    switch (json.type())
    {
        case Json::nullValue:
        ...
        case Json::stringValue: {
            ...
            // actually a time. First check if it looks anything like a time
            // string.
...
bool static has_only_string_keys(value_map const& map)
{
...
static void
write_json_value(Json::Value* json, value const& v)
{
    switch (v.type())
    {
        case value_type::NIL:
        ...
        case value_type::MAP: {
            ...
            if (has_only_string_keys(x))
            {
                *json = Json::Value(Json::objectValue);
                for (value_map::const_iterator i = x.begin(); i != x.end();
                     ++i)
                    write_json_value(
                        &(*json)[cast<string>(i->first)], i->second);
            }
...
void
value_to_json(string* json, value const& v)
{
    Json::Value root;
...
blob
value_to_json_blob(value const& v)
{
...
void static read_msgpack_value(
    value* v, ownership_holder const& ownership, msgpack::object const& object)
{
    switch (object.type)
    {
        ...
        case msgpack::type::ARRAY: {
            ...
            for (size_t i = 0; i != n_elements; ++i)
                read_msgpack_value(
                    &list[i], ownership, object.via.array.ptr[i]);
            v->swap_in(list);
            break;
        }
...
void
parse_msgpack_value(value* v, uint8_t const* data, size_t size)
{
...
// directly.
bool static msgpack_unpack_reference_type(
...
string
value_to_msgpack_string(value const& v)
{
...
blob
value_to_msgpack_blob(value const& v)
{
...
void static read_raw_value(raw_memory_reader& r, value& v)
{
    value_type type;
    {
        uint32_t t;
...
void static read_raw_value(value* v, uint8_t const* data, size_t size)
{
    raw_memory_reader r(data, size);
    read_raw_value(r, *v);
}

void static write_raw_value(raw_memory_writer& w, value const& v)
{
    {
...
void static write_raw_value(byte_vector* data, value const& v)
{
...
void
write_base_255_number(uint8_t* data, size_t length, uint64_t n)
{
    data += length - 1;
    *data = 0xff;
    --data;
...
uint64_t
read_base_255_number(uint8_t const*& data, size_t& size)
{
    uint64_t n = 0;
    while (1)
    {
...
void
deserialize_value(value* v, uint8_t const* data, size_t size, uint32_t* crc)
{
...
void
serialize_value(byte_vector* data, value const& v, uint32_t* crc)
{
    byte_vector raw;
...
static void
write_block(std::ofstream& f, void const* data, size_t size)
{
...
static void
read_block(std::ifstream& f, void* data, size_t size)
{
...
void
read_value_file(value* v, file_path const& file, uint32_t* crc)
{
...
void
write_value_file(file_path const& file, value const& v, uint32_t* crc)
{
    byte_vector raw;
...
void
parse_base64_value_string(value* v, string const& s, uint32_t* crc)
{
    byte_vector raw;
...
void
value_to_base64_string(string* s, value const& v, uint32_t* crc)
{
    byte_vector raw;
    serialize_value(&raw, v, crc);
    *s = base64_encode(
        &raw[0], raw.size(), get_url_friendly_base64_character_set());
}

void
initialize_parser_with_file(text_parser& p, filesystem_item const& item)
{
