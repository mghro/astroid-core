...
size_t
deep_sizeof(path const& x)
{
    return cradle::deep_sizeof(x.string());
}

void
to_value(cradle::value* v, path const& x)
{
    to_value(v, x.string());
}
void
from_value(path* x, cradle::value const& v)
{
    cradle::string s;
    from_value(&s, v);
    *x = path(s);
...
std::string
to_string(path const& x)
{
    return x.string();
}
void
from_string(path* x, std::string const& s)
{
    *x = path(s);
}
...
string
get_extension(file_path const& path)
{
    return get_extension(path.filename().string());
}

string
get_extension(string const& name)
{
...
static string
openmode_to_string(std::ios::openmode mode)
{
    string str;
...
open_file_error::open_file_error(
    file_path const& file, std::fstream::openmode mode, string const& msg)
...
c_file::c_file(file_path const& file, const char* mode)
{
    ...
    {
        throw file_error(
            file, string("unable to open file (mode '") + mode + "')");
    }
    path_ = file;
}

void
c_file::read(void* data, size_t size)
{
...
void
c_file::write(void const* data, size_t size)
{
    // MS can't deal with large blocks of data, so we need to break it up for
    // them.
    size_t max_block_size = 0x40000000; // 1GB
...
void
c_file::seek(int64_t offset, int whence)
{
#ifdef WIN32
...
void
c_file::throw_error(string const& msg)
{
    int error = ferror(f_);
    if (error != 0)
...
void
simple_file_parser::check_char(char expected)
{
    skip_space();
    if (*p_ != expected)
...
unsigned
simple_file_parser::read_unsigned()
{
    int i = read_int();
    if (i < 0)
...
double
simple_file_parser::read_double()
{
    skip_space();
    char* end;
...
string
simple_file_parser::read_string(int end_marker)
{
    skip_space();
    if (*p_ == '\0')
...
string
simple_file_parser::read_rest_of_line()
{
    char* start = p_;
    while (*p_ != '\r' && *p_ != '\n' && *p_ != '\0')
        ++p_;
...
string
simple_file_parser::read_identifier()
{
    skip_space();
    if (!isalpha(*p_) && *p_ != '_')
...
void
simple_file_parser::throw_error(string const& msg) const
{
    throw file_error(
        file_path_,
        (boost::format("%i:%i: error: %s") % line_n_ % (p_ - &buffer_[0])
         % msg)
            .str());
}

void
simple_file_parser::throw_unexpected_char() const
{
    if (*p_ == '\0')
    {
        throw_error("unexpected EOL");
    ...
    {
        throw_error((boost::format("unexpected character: %c (0x%02x)") % *p_
                     % static_cast<int>(*p_))
                        .str());
    }
}
