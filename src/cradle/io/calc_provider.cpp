...
void static check_error(boost::system::error_code const& error)
{
    if (error)
...
bool static has_incoming_message(calc_provider& provider)
{
...
void static post_message(
    internal_message_queue& queue, calc_provider_message&& message)
{
...
void static perform_calculation(
    internal_message_queue& queue,
    ...
    calc_supervisor_calculation_request const& request)
{
    ...
    catch (std::exception& e)
    {
        post_message(
            queue,
            make_calc_provider_message_with_failure(calc_provider_failure(
...
bool static transmit_queued_messages(
    tcp::socket& socket, internal_message_queue& queue)
{
...
void static dispatch_and_monitor_calculation(
    ...
    calc_supervisor_calculation_request const& request)
{
    ...
    boost::thread thread{[&]() { perform_calculation(queue, api, request); }};
    ...
    while (true)
    {
        ...
        {
            auto message = read_message<calc_supervisor_message>(
            ...
            switch (message.type)
            {
                ...
                case calc_supervisor_message_type::PING:
                    write_message(
                        provider.socket,
                        ipc_version,
                        make_calc_provider_message_with_pong(
                            as_ping(message)));
                    break;
            }
        }
        ...
        switch (message.type)
        {
            case calc_supervisor_message_type::FUNCTION:
                dispatch_and_monitor_calculation(
                    provider, api, as_function(message));
                break;
            case calc_supervisor_message_type::PING:
                write_message(
                    provider.socket,
