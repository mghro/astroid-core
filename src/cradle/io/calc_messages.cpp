    ...
    switch (static_cast<calc_message_code>(code))
    {
        case calc_message_code::FUNCTION: {
            ...
            for (uint16_t i = 0; i != n_args; ++i)
            {
                ...
                parse_msgpack_value(
                    &request.args[i], ownership, reader.buffer, arg_length);
        ...
        case calc_message_code::PING:
            *message = make_calc_supervisor_message_with_ping(
...
byte_vector static serialize_message(calc_provider_message const& message)
{
    byte_vector buffer;
    ...
    switch (message.type)
    {
        ...
        case calc_provider_message_type::RESULT:
            // This could be very large, so we don't ever want to keep the
            // whole serialized form in memory.
            throw exception(
                "cannot write calc provider RESULT message to buffer");
        case calc_provider_message_type::FAILURE: {
...
{
    msgpack_asio_buffer(tcp::socket& socket) : socket_(socket)
    {
    }
...
void static stream_value(tcp::socket& socket, value const& x)
{
...
void
write_message_body(tcp::socket& socket, calc_provider_message const& message)
{
    if (message.type == calc_provider_message_type::RESULT)
    {
    ...
    else
    {
        ...
        boost::asio::write(
            socket, boost::asio::buffer(&buffer[0], buffer.size()));
    }
}
