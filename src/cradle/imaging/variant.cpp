...
template<unsigned N>
void
variant_from_value(image<N, variant, shared>* x, value const& v)
{
...
template<unsigned N>
void
variant_to_value(value* v, image<N, variant, shared> const& y)
{
    ...
    b.size = product(x.size) * get_channel_size(x.pixels.type_info.type)
             * get_channel_count(x.pixels.type_info.format);
...
template<unsigned N>
void
read_variant_fields_from_immutable_map(
    image<N, variant, shared>* x, std::map<string, untyped_immutable> const& r)
{
...
get_variant_type_info(image<N, variant, shared> const& x)
{
    ...
    fields.push_back(cradle::raw_structure_field_info(
        "size", "the size (in pixels) of the image", get_type_info(x.size)));
    fields.push_back(cradle::raw_structure_field_info(
        "origin",
    ...
    fields.push_back(cradle::raw_structure_field_info(
        "pixels",
        "the array of raw pixel values (d)",
        cradle::raw_type_info(
            cradle::raw_kind::SIMPLE,
...
#define CRADLE_DEFINE_REGULAR_IMAGE_INTERFACE(T)                              \
    ...
    void read_fields_from_immutable_map(                                      \
        T& x, std::map<string, untyped_immutable> const& v)                   \
    {                                                                         \
    ...
    namespace std {                                                           \
    size_t                                                                    \
    hash<T>::operator()(T const& x) const                                     \
    {                                                                         \
