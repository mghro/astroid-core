...
{
    ptr.ownership
        = std::shared_ptr<Pixel>(pixels, releasable_array_deleter<Pixel>());
    ptr.view = pixels;
}
...
template<class Pixel>
bool
has_sole_ownership(shared_pointer<Pixel> const& ptr)
{
    auto* owner = any_cast<std::shared_ptr<Pixel>>(ptr.ownership);
...
template<class Pixel>
Pixel*
release_ownership(shared_pointer<Pixel>& ptr)
{
    assert(has_sole_ownership(ptr));
    auto* owner = const_cast<std::shared_ptr<Pixel>*>(
        any_cast<std::shared_ptr<Pixel>>(ptr.ownership));
...
#define CRADLE_DECLARE_REGULAR_IMAGE_INTERFACE(N, T)                          \
    ...
    {                                                                         \
        return raw_type_info(                                                 \
            raw_kind::NAMED_TYPE_REFERENCE,                                   \
            any(raw_named_type_reference(                                     \
