...
void
check_slice_consistency(std::vector<image_slice<N, T, SP>> const& slices)
{
    image_slice<N, T, SP> const& slice0 = slices.front();
    for (typename std::vector<image_slice<N, T, SP>>::const_iterator i
         = slices.begin();
         i != slices.end();
         ++i)
    {
...
struct image_slice_comparison
{
    bool
    operator()(
        image_slice<N, T, SP> const& a, image_slice<N, T, SP> const& b) const
    {
    ...
    regular_grid<1, double> const& interpolation_grid)
{
    ...
    if (slices.empty())
    {
        throw cradle::exception("merge_slices() called with empty slice list");
    }
    ...
    vector<N + 1, unsigned> dimensions
        = unslice(slice0.content.size, axis, interpolation_grid.n_points[0]);
    ...
    tmp.axes[axis] = unslice(
        uniform_vector<N, double>(0), axis, interpolation_grid.spacing[0]);
    ...
    // immediately after.
    typename std::vector<image_slice<N, T, SP>>::const_iterator prev_real_slice
        = slices.begin();
    typename std::vector<image_slice<N, T, SP>>::const_iterator next_real_slice
        = slices.begin();
    ...
    for (unsigned i = 0; i < interpolation_grid.n_points[0]; ++i)
    {
        ...
        else
        {
            ...
            raw_blend_images(
                merged_slice.content,
                prev_real_slice->content,
                next_real_slice->content,
                fraction);
        }
    ...
    std::vector<image_slice<N, T, SP>> const& slices)
{
    ...
    auto interpolation_grid = compute_interpolation_grid(
        map([](image_slice<N, T, SP> const& slice)
                -> double { return slice.position; },
            sorted_slices),
        4.);
...
struct merge_images_fn
{
    ...
    void operator()(Pixel)
    {
        ...
        for (std::size_t i = 0; i != n_slices; ++i)
        {
            ...
            dst.content = cast_image<image<N, Pixel, const_view>>(src.content);
