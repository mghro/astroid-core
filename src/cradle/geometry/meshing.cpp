    ...
    bool setDataInside)
{
    ...
    //    else { compute_bounding_box(mesh_bounds,
    //    bounding_box(meshes.at(i).mesh)); }
    //}
    ...
    for (unsigned int k = 0; k < img.size[2]; ++k)
    {
        ...
        for (unsigned int j = 0; j < img.size[1]; ++j)
        {
            ...
            for (unsigned int i = 0; i < img.size[0]; ++i)
            {
                ...
                bool isInside = false;
                size_t n = 0;
                while (!isInside && n < meshes.size())
                {
                    isInside
                        = mesh_contains(meshes.at(n++), make_vector(x, y, z));
                }
...
make_optimized_triangle_mesh_for_structure(structure_geometry const& structure)
{
