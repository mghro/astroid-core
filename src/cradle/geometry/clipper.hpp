...
void
to_value(value* v, clipper_poly const& x);
void
from_value(clipper_poly* x, value const& v);
void
to_value(value* v, clipper_polyset const& x);
void
from_value(clipper_polyset* x, value const& v);
...
{
    size_t
    operator()(cradle::clipper_polyset const& x) const
    {
        return cradle::invoke_hash(cradle::from_clipper(x));
    }
};
} // namespace std
namespace cradle {
