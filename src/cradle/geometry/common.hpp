...
namespace std {
template<unsigned N, class T>
struct hash<cradle::vector<N, T>>
{
    size_t
    operator()(cradle::vector<N, T> const& v) const
    {
        size_t h = 0;
        for (unsigned i = 0; i != N; ++i)
            h = cradle::combine_hashes(h, hash<T>()(v[i]));
        return h;
    }
};
} // namespace std
...
{
    return cradle::box<N, T>(box.corner - border, box.size + border * 2);
}
...
namespace std {
template<unsigned M, unsigned N, class T>
struct hash<cradle::matrix<M, N, T>>
{
    size_t
    operator()(cradle::matrix<M, N, T> const& m) const
    {
        size_t h = 0;
        for (unsigned i = 0; i != M; ++i)
        {
            for (unsigned j = 0; j != N; ++j)
                h = cradle::combine_hashes(h, hash<T>()(m(i, j)));
        }
        return h;
    }
};
    ...
    vector<N, Value> const& a, vector<N, Value> const& b, Tolerance tolerance)
{
    for (unsigned i = 0; i < N; ++i)
    {
        if (!cradle::almost_equal(a[i], b[i], static_cast<Value>(tolerance)))
        {
            return false;
        }
    }
...
template<unsigned M, unsigned N, class T>
void
from_value(matrix<M, N, T>* x, cradle::value const& v)
{
...
template<unsigned M, unsigned N, class T>
void
to_value(cradle::value* v, matrix<N, M, T> const& x)
{
...
{
    size_t
    operator()(cradle::line_segment<N, T> const& x) const
    {
...
{
    size_t
    operator()(cradle::triangle<N, T> const& x) const
    {
        return cradle::combine_hashes(
