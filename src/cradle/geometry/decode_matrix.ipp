...
match_element(
    ...
    IteratorT end,
    IteratorT value,
    element<T> const& pattern,
    T epsilon)
{
    switch (pattern.type)
    {
        case element<T>::CONSTANT:
            return almost_equal(*value, pattern.data.constant, epsilon);
...
bool
match(
    matrix<M, N, T> const& values,
    matrix<M, N, element<T>> const& pattern,
    T tolerance = default_equality_tolerance<T>())
{
...
decode_rotation_about_x(matrix<3, 3, T> const& m)
{
    ...
    matrix<3, 3, element<T>> p(
        one, zero, zero, zero, same_as<T>(+4), negative_of<T>(+2), zero, x, x);
    if (!match(m, p))
...
decode_rotation_about_y(matrix<3, 3, T> const& m)
{
    ...
    element<T> zero = constant<T>(0), one = constant<T>(1), x = dont_care<T>();
    matrix<3, 3, element<T>> p(
        same_as<T>(+8), zero, negative_of<T>(+4), zero, one, zero, x, zero, x);
    if (!match(m, p))
        return optional<angle<T, radians>>();
...
decode_rotation_about_z(matrix<3, 3, T> const& m)
{
    ...
    element<T> zero = constant<T>(0), one = constant<T>(1), x = dont_care<T>();
    matrix<3, 3, element<T>> p(
        same_as<T>(+4), negative_of<T>(+2), zero, x, x, zero, zero, zero, one);
    if (!match(m, p))
        return optional<angle<T, radians>>();
