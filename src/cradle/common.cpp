...
type_mismatch::type_mismatch(value_type expected, value_type got)
    : exception(
        str(boost::format("type mismatch\n    "
                          "expected: %s\n    got: %s")
            % expected % got)),
      expected_(expected),
      got_(got)
...
static void
write_escaped_string(std::ostream& s, string const& x)
{
    s << '\"';
    for (auto c : x)
    {
...
struct value_ostream_operator
{
    ...
    void
    operator()(boost::posix_time::ptime const& v)
    {
        s << v;
    }
    void
    operator()(value_list const& v)
    {
        s << "[";
...
static void
check_type(value_type got, value_type expected)
{
    if (got != expected)
...
void
value::get(value_map const** v) const
{
...
void
value::set(string const& v)
{
    type_ = value_type::STRING;
    value_ = v;
}
void
value::set(blob const& v)
{
    type_ = value_type::BLOB;
    value_ = v;
}
void
value::set(boost::posix_time::ptime const& v)
{
    type_ = value_type::DATETIME;
    value_ = v;
}
void
value::set(value_list const& v)
{
    type_ = value_type::LIST;
    value_ = v;
}
void
value::set(value_map const& v)
{
...
void
value::swap_in(value_list& v)
{
    this->set(v);
}
void
value::swap_in(value_map& v)
{
    this->set(v);
}
...
struct value_deep_sizeof_operator
{
    ...
    void
    operator()(Contents const& contents)
    {
...
namespace std {
size_t
hash<cradle::value>::operator()(cradle::value const& x) const
{
    cradle::hash_fn fn;
...
#define CRADLE_DEFINE_SMALL_INTEGER_INTERFACE(T)                              \
    ...
    {                                                                         \
        return raw_type_info(                                                 \
            raw_kind::SIMPLE, any(raw_simple_type::INTEGER));                 \
    }                                                                         \
...
operator<<(std::ostream& s, resolved_function_uid const& id)
{
    ...
    s << base64_encode(
        reinterpret_cast<uint8_t const*>(json.c_str()),
        json.length(),
...
immutable_data_type_mismatch::immutable_data_type_mismatch(
    ...
    : exception(
        "type mismatch\n"
        "expected type: "
        + to_string(make_api_type_info(expected))
...
size_t static hash_request(untyped_request const& request)
{
    size_t content_hash;
    switch (request.type)
    {
        ...
        case request_type::PROPERTY: {
            ...
            content_hash = combine_hashes(
                invoke_hash(property.record), invoke_hash(property.field));
            break;
        }
...
operator==(untyped_request const& a, untyped_request const& b)
{
    ...
    switch (a.type)
    {
        ...
        case request_type::REQUIRED:
            return as_required(a).optional_value
                   == as_required(b).optional_value;
        case request_type::ISOLATED:
