...
get_type_info(units const&)
{
    return cradle::raw_type_info(
        cradle::raw_kind::SIMPLE, any(cradle::raw_simple_type::STRING));
}
...
static inline void
swap(units& a, units& b)
{
    swap(a.name, b.name);
}

static inline bool
operator==(units const& a, units const& b)
{
...
static void
to_value(value* v, units const& x)
{
    to_value(v, x.name);
}
static void
from_value(units* x, value const& v)
{
    from_value(&x->name, v);
}

static std::ostream&
operator<<(std::ostream& s, units const& x)
{
    s << x.name;
    return s;
}
...
{
    size_t
    operator()(cradle::units const& x) const
    {
        return cradle::invoke_hash(x.name);
    }
};
} // namespace std
namespace cradle {
