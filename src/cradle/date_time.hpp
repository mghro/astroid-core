...
void
to_value(value* v, time const& x);
void
from_value(time* x, value const& v);
...
static inline cradle::raw_type_info get_type_info(date)
{
...
static inline cradle::raw_type_info get_type_info(ptime)
{
    return cradle::raw_type_info(
        cradle::raw_kind::SIMPLE,
...
{
    size_t
    operator()(cradle::date const& x) const
    {
...
{
    size_t
    operator()(boost::gregorian::greg_day x) const
    {
        return size_t(x);
    }
};
...
{
    size_t
    operator()(cradle::time const& x) const
    {
        return cradle::invoke_hash(cradle::to_string(x));
    }
};
