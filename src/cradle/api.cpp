        ...
        info.fields[field.name]
            = field.type.kind == raw_kind::OMISSIBLE
                  ? api_structure_field_info(
                      ...
                      some(true))
                  : api_structure_field_info(
                      field.description, make_api_type_info(field.type), none);
    }
    return info;
}

api_union_info static make_api_union_info(raw_union_info const& raw)
{
    api_union_info info;
    for (auto const& member : raw.members)
    {
        info.members[member.name] = api_union_member_info(
            member.description, make_api_type_info(member.type));
    }
    return info;
}

api_enum_info static make_api_enum_info(raw_enum_info const& raw)
{
    api_enum_info info;
    for (auto const& value : raw.values)
    {
        info.values[value.name] = api_enum_value_info(value.description);
    }
    return info;
}

api_type_info static make_api_simple_type(raw_simple_type raw)
{
    switch (raw)
    {
...
api_type_info
make_api_type_info(raw_type_info const& raw)
{
    switch (raw.kind)
    {
        case raw_kind::ARRAY: {
            auto const& raw_info = unsafe_any_cast<raw_array_info>(raw.info);
            return make_api_type_info_with_array_type(api_array_info(
                raw_info.size, make_api_type_info(raw_info.element_type)));
        }
        case raw_kind::ENUM: {
        ...
        case raw_kind::MAP: {
            ...
            return make_api_type_info_with_map_type(api_map_info(
        ...
        case raw_kind::STRUCTURE: {
            auto const& raw_info
                = unsafe_any_cast<raw_structure_info>(raw.info);
...
{
    api
    .dependencies.push_back(api_dependency_type_info(account, app, version));
}

void
register_api_provider_type(api_implementation& api, string const& tag)
{
...
void
register_api_previous_release_version(
    api_implementation& api, string const& version)
{
...
bool
function_is_upgrade(
    std::vector<api_upgrade_type_info> const& upgrades, string function_name)
{
