...
value_diff_item static make_insert_item(
    value_diff_path const& path, value const& new_value)
{
...
value_diff_item static make_update_item(
    value_diff_path const& path, value const& new_value)
{
    value_diff_item item;
    item.path = path;
...
value_diff_item static make_delete_item(value_diff_path const& path)
{
    value_diff_item item;
    item.path = path;
...
value_diff_path static extend_path(
    value_diff_path const& path, value const& addition)
{
...
void static compute_value_diff(
    value_diff& diff,
    value_diff_path const& path,
    value const& a,
    value const& b);

void static compute_map_diff(
    value_diff& diff,
    value_diff_path const& path,
    value_map const& a,
    value_map const& b)
    ...
    value_list const& b)
{
        ...
        if (removal)
        {
            for (size_t i = get(removal).count; i != 0; --i)
            {
                diff.push_back(make_delete_item(
                    extend_path(path, to_value(get(removal).index + i - 1))));
            }
            return;
        }
    }
    ...
    else
    {
        ...
        {
            compute_value_diff(
                diff, extend_path(path, to_value(i)), a[i], b[i]);
        }
        return;
    }
...
value_diff
compute_value_diff(value const& a, value const& b)
{
    value_diff diff;
...
value static apply_value_diff_item(
    value const& initial,
    value_diff_path const& path,
    size_t path_index,
    value_diff_op op,
    ...
    switch (path_element.type())
    {
        ...
        case value_type::INTEGER: {
            ...
            else
            {
                if (index >= list.size())
                    throw invalid_diff_path();
                list[index] = apply_value_diff_item(
                    list[index], path, path_index + 1, op, new_value);
            }
            return value(list);
        }
        default:
...
value
apply_value_diff(value const& v, value_diff const& diff)
{
    value patched = v;
