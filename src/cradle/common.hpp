...
namespace std {
template<class Tag>
struct hash<cradle::flag_set<Tag>>
{
    size_t
    operator()(cradle::flag_set<Tag> const& set) const
    {
...
any_cast(any const& a)
{
    typed_cloneable_value_holder<T> const* ptr
        = dynamic_cast<typed_cloneable_value_holder<T> const*>(
            get_value_pointer(a));
...
{
    assert(dynamic_cast<typed_cloneable_value_holder<T> const*>(
        get_value_pointer(a)));
    return static_cast<typed_cloneable_value_holder<T> const*>(
               get_value_pointer(a))
        ->value;
}
template<class T>
...
unsafe_any_cast(any& a)
{
    assert(
        dynamic_cast<typed_cloneable_value_holder<T>*>(get_value_pointer(a)));
    return static_cast<typed_cloneable_value_holder<T>*>(get_value_pointer(a))
        ->value;
}
...
struct any_by_ref
{
    ...
    any_by_ref&
    operator=(any_by_ref const& other)
    {
        holder_ = other.holder_;
        return *this;
    }
    ...
    std::shared_ptr<untyped_value_holder> holder_;
};
static inline void
swap(any_by_ref& a, any_by_ref& b)
{
...
template<class T>
T const&
unsafe_any_cast(any_by_ref const& a)
{
    assert(dynamic_cast<typed_value_holder<T> const*>(get_value_pointer(a)));
    return static_cast<typed_value_holder<T> const*>(get_value_pointer(a))
        ->value;
}
...
struct raw_type_info
{
    ...
    raw_type_info(raw_kind kind, any const& info) : kind(kind), info(info)
    {
    }
};
...
struct raw_structure_field_info
{
    ...
    raw_structure_field_info(
        string const& name, string const& description, raw_type_info type)
...
struct raw_enum_value_info
{
    ...
    raw_enum_value_info(string const& name, string const& description)
...
struct raw_array_info
{
    ...
    raw_array_info(
        optional<unsigned> const& size, raw_type_info const& element_type)
...
struct raw_map_info
{
    ...
    raw_map_info(raw_type_info const& key, raw_type_info const& value)
        : key(key), value(value)
    {
    }
};
...
struct raw_named_type_reference
{
    ...
    raw_named_type_reference(string const& app, string const& type)
        : app(app), type(type)
    {
    }
};
...
std::istream&
operator>>(std::istream& s, value_type& t);
...
static inline bool
operator==(nil_type a, nil_type b)
{
    return true;
}
static inline bool
...
operator<<(std::ostream& s, nil_type x)
{
    return s << "nil";
}
...
{
    size_t
    operator()(cradle::nil_type x) const
    {
        return 0;
    }
};
...
{
    size_t
    operator()(cradle::blob const& x) const
    {
        // TODO: hash for real?
        return 0;
    }
...
value
get_field(value_map const& r, string const& field);

// This is the same as above, but its return value indicates whether or not
// the field is in the record.
bool
get_field(value* v, value_map const& r, string const& field);
...
value const&
get_union_value_type(value_map const& map);
...
struct value
{
    ...
    void
    get(bool const** v) const;
    void
    get(integer const** v) const;
    void
    get(double const** v) const;
    void
    get(string const** v) const;
    void
    get(blob const** v) const;
    void
    get(boost::posix_time::ptime const** v) const;
    void
    get(value_list const** v) const;
    void
    get(value_map const** v) const;
    ...
    void
    set(value_list const& v);
    void
    set(value_map const& v);

    // potentially more efficient setters which consume their arguments
    void
    swap_in(value_list& v);
    void
    swap_in(value_map& v);

    void
    swap_with(value& other);
...
read_field_from_record(
    Field* field_value, value_map const& record, string const& field_name)
{
...
write_field_to_record(
    value_map& record, string const& field_name, Field const& field_value)
{
    to_value(&record[value(field_name)], field_value);
}

static inline raw_type_info
get_type_info(value const&)
{
...
void
value_to_string(string* s, value const& v);
...
static inline void
to_value(value* v, value const& x)
{
    *v = x;
}
static inline void
from_value(value* x, value const& v)
{
    *x = v;
}
...
template<class T>
std::ostream&
generic_ostream_operator(std::ostream& s, T const& x)
{
    value v;
    to_value(&v, x);
    string t;
...
{
    invalid_enum_string(raw_enum_info const& enum_type, string const& value)
...
{
    invalid_enum_value(raw_enum_info const& enum_type, unsigned value)
        : exception(
            "invalid " + enum_type.name + " value: " + to_string(value))
    {
    }
};
struct index_out_of_bounds : exception
{
    index_out_of_bounds(char const* label, size_t value, size_t upper_bound)
        : exception(
            string("index out of bounds: ") + label + "; value: "
            + to_string(value) + "; upper bound: " + to_string(upper_bound))
    {
    }
};
...
void
check_index_bounds(char const* label, size_t index, size_t upper_bound);
...
template<class Fn>
void
apply_fn_to_value(Fn& fn, value const& v)
{
    switch (v.type())
    {
        case value_type::NIL:
...
template<class Fn>
void
apply_fn_to_value_pair(Fn& fn, value const& a, value const& b)
{
    if (a.type() != b.type())
    ...
    switch (a.type())
    {
        ...
        case value_type::BLOB:
            ...
            break;
        case value_type::DATETIME:
            fn(cast<boost::posix_time::ptime>(a),
...
{
    size_t
    operator()(cradle::value const& x) const;
};
} // namespace std
namespace cradle {
...
std::ostream&
operator<<(std::ostream& s, value const& v);

void
parse_value_string(value* v, string const& s);
void
value_to_string(string* s, value const& v);
...
static inline void
to_value(value* v, nil_type n)
{
    v->set(n);
}
static inline void
from_value(nil_type* n, value const& v)
{
}
...
{
    size_t
    operator()(vector<Item> const& x) const
    {
        size_t h = 0;
        for (auto const& i : x)
...
template<class T>
std::ostream&
operator<<(std::ostream& s, std::vector<T> const& x)
{
    s << "{";
    size_t n_elements = x.size();
...
template<class T>
raw_type_info
get_type_info(std::vector<T> const&)
{
    return raw_type_info(
        raw_kind::ARRAY, any(raw_array_info(none, get_type_info(T()))));
}
...
{
    size_t
    operator()(map<Key, Value> const& x) const
    {
        size_t h = 0;
        for (auto const& i : x)
        {
            h = cradle::combine_hashes(
                h,
                cradle::combine_hashes(
                    cradle::invoke_hash(i.first),
                    cradle::invoke_hash(i.second)));
        }
        return h;
    }
};
...
template<class Key, class Value>
std::ostream&
operator<<(std::ostream& s, std::map<Key, Value> const& x)
{
    s << "{";
    for (typename std::map<Key, Value>::const_iterator i = x.begin();
         i != x.end();
         ++i)
    {
        if (i != x.begin())
...
template<class Key, class Value>
raw_type_info
get_type_info(std::map<Key, Value> const&)
{
    return raw_type_info(
        raw_kind::MAP,
...
#define CRADLE_REGULAR_TYPE_WRAPPER(templating, Wrapper, field)               \
    templating void to_value(cradle::value* v, Wrapper const& x)              \
    {                                                                         \
        to_value(v, x.field);                                                 \
        templating void from_value(Wrapper* x, cradle::value const& v)        \
        {                                                                     \
            from_value(&x->field, v);                                         \
            templating bool operator==(Wrapper const& x, Wrapper const& y)    \
            {                                                                 \
...
get_type_info(optional<T> const&)
{
    using cradle::get_type_info;
    return cradle::raw_type_info(
        cradle::raw_kind::OPTIONAL, cradle::any(get_type_info(T())));
}
template<class T>
size_t
...
template<class T>
std::ostream&
operator<<(std::ostream& s, optional<T> const& x)
{
    if (x)
        s << get(x);
    else
...
namespace std {
template<class T>
struct hash<cradle::optional<T>>
{
    size_t
    operator()(cradle::optional<T> const& x) const
    {
        return x ? cradle::invoke_hash(get(x)) : 0;
    }
};
} // namespace std
namespace cradle {
...
cradle::raw_type_info get_type_info(vector<N, T>)
{
    using cradle::get_type_info;
    return cradle::raw_type_info(
        cradle::raw_kind::ARRAY,
        cradle::any(cradle::raw_array_info(N, get_type_info(T()))));
}
template<unsigned N, class T>
size_t
deep_sizeof(vector<N, T> const& x)
{
...
template<unsigned N, class T>
void
from_value(vector<N, T>* x, cradle::value const& v)
{
...
template<unsigned N, class T>
void
to_value(cradle::value* v, vector<N, T> const& x)
{
...
template<class T>
cradle::raw_type_info
get_type_info(omissible<T> const&)
{
    using cradle::get_type_info;
    return cradle::raw_type_info(
        cradle::raw_kind::OMISSIBLE, cradle::any(get_type_info(T())));
}
template<class T>
size_t
...
template<class T>
void
to_value(cradle::value* v, omissible<T> const& x)
{
    cradle::value_map record;
    if (x)
    {
...
template<class T>
void
from_value(omissible<T>* x, cradle::value const& v)
{
    cradle::value_map const& record = cradle::cast<cradle::value_map>(v);
    string type;
    from_value(&type, cradle::get_union_value_type(record));
...
{
    size_t
    operator()(cradle::omissible<T> const& x) const
    {
        return x ? cradle::invoke_hash(get(x)) : 0;
    }
};
...
template<unsigned N, class T>
raw_type_info
get_type_info(c_array<N, T> const&)
{
    return raw_type_info(
        raw_kind::ARRAY, any(raw_array_info(N, get_type_info(T()))));
}
template<unsigned N, class T>
size_t
...
template<unsigned N, class T>
void
from_value(c_array<N, T>* x, value const& v)
{
    value_list const& l = cast<value_list>(v);
...
template<unsigned N, class T>
void
to_value(value* v, c_array<N, T> const& x)
{
    value_list l(N);
    for (unsigned i = 0; i != N; ++i)
        to_value(&l[i], x[i]);
...
{
    size_t
    operator()(cradle::c_array<N, T> const& x) const
    {
        size_t h = 0;
...
{
    std::shared_ptr<T> ptr(new T[n_elements], array_deleter<T>());
    array->ownership = ptr;
...
template<class T>
void
from_value(array<T>* x, value const& v)
{
...
template<class T>
void
to_value(value* v, array<T> const& x)
{
    blob b;
...
{
    size_t
    operator()(cradle::array<T> const& x) const
    {
        size_t h = 0;
        for (auto const& i : x)
            h = cradle::combine_hashes(h, cradle::invoke_hash(i));
...
{
    size_t
    operator()(cradle::immutable<T> const& x) const
    {
...
template<class T>
void
from_value(immutable<T>* x, cradle::value const& v)
{
    using cradle::from_value;
    x->ptr.reset(new immutable_value<T>);
...
template<class T>
void
to_value(cradle::value* v, immutable<T> const& x)
{
    using cradle::to_value;
    if (x.ptr)
        to_value(v, get(x));
...
struct immutable_data_type_mismatch : exception
{
 ...
 private:
    std::shared_ptr<raw_type_info> expected_, got_;
};
...
template<class T>
size_t
deep_sizeof(object_reference<T> const& x)
{
...
template<class T>
void
swap(object_reference<T>& a, object_reference<T>& b)
{
    swap(a.uid, b.uid);
}
...
template<class T>
bool
operator==(object_reference<T> const& a, object_reference<T> const& b)
{
    return a.uid == b.uid;
}
template<class T>
...
template<class T>
void
from_value(object_reference<T>* x, cradle::value const& v)
{
    using cradle::from_value;
    from_value(&x->uid, v);
}
template<class T>
void
to_value(cradle::value* v, object_reference<T> const& x)
{
    using cradle::to_value;
    to_value(v, x.uid);
}
...
template<class T>
std::ostream&
operator<<(std::ostream& s, object_reference<T> const& x)
{
    s << x.uid;
    return s;
}
...
{
    size_t
    operator()(cradle::object_reference<T> const& x) const
    {
        return cradle::invoke_hash(x.uid);
    }
};
} // namespace std
namespace cradle {
...
template<class T>
size_t
deep_sizeof(immutable_reference<T> const& x)
{
    return deep_sizeof(x.uid);
}
...
template<class T>
void
swap(immutable_reference<T>& a, immutable_reference<T>& b)
{
    swap(a.uid, b.uid);
}
...
template<class T>
bool
operator==(immutable_reference<T> const& a, immutable_reference<T> const& b)
{
    return a.uid == b.uid;
}
template<class T>
...
template<class T>
void
from_value(immutable_reference<T>* x, cradle::value const& v)
{
    using cradle::from_value;
    from_value(&x->uid, v);
}
template<class T>
void
to_value(cradle::value* v, immutable_reference<T> const& x)
{
    using cradle::to_value;
    to_value(v, x.uid);
}
...
template<class T>
std::ostream&
operator<<(std::ostream& s, immutable_reference<T> const& x)
{
    s << x.uid;
    return s;
}
...
{
    size_t
    operator()(cradle::immutable_reference<T> const& x) const
    {
        return cradle::invoke_hash(x.uid);
    }
};
...
template<class T>
void
ensure_default_initialization(T& x)
{
}
static inline void
ensure_default_initialization(bool& x)
{
    x = false;
}
#define CRADLE_DECLARE_NUMBER_INITIALIZER(T)                                  \
    static inline void ensure_default_initialization(T& x)                    \
    {                                                                         \
        x = 0;                                                                \
    }
...
std::ostream&
operator<<(std::ostream& s, resolved_function_uid const& id);
...
{
    size_t
    operator()(cradle::resolved_function_uid const& x) const
    {
        return cradle::combine_hashes(
            cradle::invoke_hash(x.app), cradle::invoke_hash(x.uid));
    }
};
} // namespace std
namespace cradle {
...
{
    static dynamic_type_interface const*
    invoke()
    {
...
// Returns greater upgrade value as determined by the enum values from
// upgrade_type This is used to parse down through the types so structs that
// have a field that has been modified will be upgraded.
upgrade_type static merged_upgrade_type(upgrade_type a, upgrade_type b)
{
    return a > b ? a : b;
}

// Gets the explicit update type for values. This is the default implementation
// of this and it's expected to be overridden by the user for any type they are
// defining an upgrade for.
template<class T>
upgrade_type
...
#define upgrade_struct(T)                                                     \
    upgrade_type static get_explicit_upgrade_type(T const&)                   \
    {                                                                         \
    ...
    object_reference<T> const&, std::vector<std::type_index> parsed_types)
{
    ...
    // attempt to get the data associated with the object_reference id
    return upgrade_type::NONE;
}
...
template<class T>
T
auto_upgrade_value_for_container(value const& v)
{
    T x;
    upgrade_value(&x, v);
    return x;
...
{
    // TODO: figure out how to upgrade array values, you don't know size that
...
struct untyped_request
{
    ...
    // If type is IMMUTABLE, this is the immutable ID of the data (as a
    // string).
    any_by_ref contents;
...
{
    size_t
    operator()(cradle::untyped_request const& request) const
    {
        return request.hash;
    }
};
...
template<class Result>
bool
operator==(request<Result> const& a, request<Result> const& b)
{
...
replace_request_contents(
    untyped_request const& request, Contents const& new_contents)
{
    return make_untyped_request(
        request.type, new_contents, request.result_interface);
}
...
make_typed_request(request_type type, Contents const& contents)
{
    ...
    typed.untyped = make_untyped_request(
        type, contents, dynamic_type_interface_maker<Result>::invoke());
    return typed;
}
...
struct structure_request_info
{
    ...
    structure_request_info(
        ...
        structure_constructor_interface const* constructor)
        : fields(fields), constructor(constructor)
    {
    }
};
...
struct property_request_info
{
    ...
    property_request_info(
        ...
        string const& field,
        field_extractor_interface const* extractor)
        : record(record), field(field), extractor(extractor)
    {
    }
};
...
struct union_request_info
{
    ...
    string member_name;
    std::shared_ptr<union_constructor_interface> constructor;
    ...
    union_request_info(
        ...
        std::shared_ptr<union_constructor_interface> const& constructor)
...
struct union_constructor : union_constructor_interface
{
    ...
    union_constructor(constructor_type constructor) : constructor(constructor)
    {
    }
...
struct required_request_info
{
    ...
    required_request_info(
        ...
        optional_unwrapper_interface const* unwrapper)
        : optional_value(optional_value), unwrapper(unwrapper)
    {
    }
};
...
{
    return make_typed_request<Value>(
        request_type::REMOTE_CALCULATION, local.untyped);
}
...
{
    return make_typed_request<Value>(
        request_type::IMMEDIATE, erase_type(make_immutable(value)));
}
...
{
    std::vector<untyped_request> untyped_item_requests = map(
        [](request<Value> const& i) { return i.untyped; }, item_requests);
    return make_typed_request<std::vector<Value>>(
...
{
    return make_typed_request<optional<Value>>(
        request_type::SOME,
        some_request_info(
            value_request.untyped, get_optional_wrapper<Value>()));
}
...
rq_required(request<optional<Value>> const& optional_request)
{
    return make_typed_request<Value>(
        ...
        required_request_info(
            optional_request.untyped, get_optional_unwrapper<Value>()));
}
...
rq_structure(std::map<string, untyped_request> const& field_requests)
{
    return make_typed_request<Value>(
        ...
        structure_request_info(
            field_requests, cradle::get_structure_constructor<Value>()));
}
...
rq_isolated(request<Value> const& wrapped)
{
    ...
    return wrapped.untyped.type == request_type::ISOLATED
               ? wrapped
