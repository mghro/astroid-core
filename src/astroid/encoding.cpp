...
static void
base36_encode(std::ostringstream& s, uint64_t id, unsigned minimum_length)
{
...
string
base36_encode(uint64_t id, unsigned minimum_length)
{
    std::ostringstream s;
...
uint64_t
base36_decode(string const& text)
{
...
static unsigned
base36_digits_required(uint64_t id)
{
    uint64_t range = 1;
    unsigned n_digits = 0;
    while (range <= id)
...
static uint64_t
scramble_id(uint64_t id, unsigned n_digits)
{
    ...
    {
        uint64_t r_next = uint64_t(nonnegative_mod(
            int64_t(l) + (int64_t(r) * 235 + 1), int64_t(half_range)));
        l = r;
        r = r_next;
    }
...
static uint64_t
descramble_id(uint64_t id, unsigned n_digits)
{
    uint64_t half_range = 1;
    for (unsigned i = 0; i != n_digits; ++i)
        half_range *= 6;
    ...
    for (unsigned i = 0; i != 6; ++i)
    {
        uint64_t l_next = uint64_t(nonnegative_mod(
            int64_t(r) - (int64_t(l) * 235 + 1), int64_t(half_range)));
        r = l;
        l = l_next;
    }
    return l * half_range + r;
...
string
nonsequential_base36_encode(uint64_t id, unsigned minimum_length)
{
...
uint64_t
nonsequential_base36_decode(string const& text)
{
...
string
ascii_to_hex(unsigned char* text, int length)
{
