...
void
lower_thread_priority(boost::thread& thread)
{
...
void
lower_thread_priority(boost::thread&)
{
}
...
struct dynamic_thread_object_assigner
{
    ...
    bool
    get_object(size_t* object_index);
    size_t
    total_objects() const
    {
        return n_objects_;
...
dynamic_thread_object_assigner::dynamic_thread_object_assigner(
    size_t n_objects)
    : n_objects_(n_objects), next_object_(0)
{
}

bool
dynamic_thread_object_assigner::get_object(size_t* object_index)
{
...
struct shared_progress_reporting_state
{
    shared_progress_reporting_state(
        ...
        : reporter(reporter), n_jobs(n_jobs), total_progress(0)
    {
    }
...
struct worker_thread_progress_reporter : progress_reporter_interface
{
    ...
    operator()(float progress)
    {
        ...
        state_->total_progress
            += (progress - progress_so_far_) / state_->n_jobs;
...
{
    worker_thread_check_in(shared_check_in_state* state) : state_(state)
    {
    }
    ...
    simple_job_interface** jobs)
{
    ...
    size_t n_threads_to_create
        = (std::min)(size_t(boost::thread::hardware_concurrency()), n_jobs);
    ...
    std::vector<worker_thread_error_report> error_reports(n_threads_to_create);
    ...
    for (size_t i = 0; i != n_threads_to_create; ++i)
    {
        worker_thread fn(
            ...
            &check_in_state,
            &error_reports[i]);
        threads.push_back(std::make_shared<boost::thread>(fn));
