#ifndef ASTROID_GEOMETRY_POLYGONAL_HPP
#define ASTROID_GEOMETRY_POLYGONAL_HPP
...
template<class Fn>
polygon2
map_points(polygon2 const& src, Fn const& fn)
{
...
api(fun trivial)
bool
polyset_empty(polyset const& set)
{
    return is_empty(set);
}
...
scale(
    ...
    polyset const& set,
    // Desired X,y scale factor
    astroid::vector<2, double> const& factor);
...
api(fun trivial)
// Distance to polyset. Points inside the polyset will return a negative
// distance
double
distance_to_polyset(
...
void
compute_bounding_box(optional<box<2, double>>& box, polyset const& set);

// Applies a mapping function to each point on the source polyset to generate
// the result polyset.
template<class Fn>
polyset
map_points(polyset const& src, Fn const& fn)
{
    polyset dst;
...
// If the polyset contains any holes, the holes will be removed from the
// polyset during expansion.
api(fun)
// Expanded polyset.
polyset
polyset_expansion(
...
// Which type of boolean operation to perform for contour/structure
// manipulation in a left-associative manner.
api(enum)
...
struct structure_geometry
{
    ...
    // Master list of all slices that this structure may potentially have
    // contours on
...
// Get the slices that are within the bounds of the given out-of-plane position
// limits.
std::vector<structure_geometry_slice>
get_structure_slices(
    structure_geometry const& structure, double p_low, double p_high);

// Constructs a structure_geometry_slice at a know position in the structure
// master list.
...
static inline bool
is_empty(structure_geometry const& structure)
{
...
api(fun trivial)
bool
structure_empty(structure_geometry const& structure)
{
