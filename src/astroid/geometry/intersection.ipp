...
intersection(astroid::ray<N, T> const& ray, astroid::box<N, T> const& box)
{
    ...
    for (unsigned i = 0; i < N; ++i)
    {
        ...
        t = (get_high_corner(box)[i] - ray.origin[i]) / ray.direction[i];
...
template<unsigned N, typename T>
optional<box<N, T>>
intersection(astroid::box<N, T> const& a, astroid::box<N, T> const& b)
{
    astroid::box<N, T> r;
    for (unsigned i = 0; i < N; ++i)
    {
...
is_intersecting(line_segment<3, T> const& segment, triangle<3, T> const& tri)
{
    ...
    if (std::fabs(eu) < 1.0e-10 || std::fabs(ev) < 1.0e-10
        || std::fabs(eu + ev - 1.0) < 1.0e-10)
    {
