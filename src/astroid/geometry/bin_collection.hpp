...
#ifndef ASTROID_GEOMETRY_BIN_COLLECTION_HPP
#define ASTROID_GEOMETRY_BIN_COLLECTION_HPP
...
struct bin_collection_item
{
    ...
    bin_collection_item(ItemType const& item, box<N, T> const& bounds)
        : item(item), bounds(bounds)
    {
    }
};

// Represents a collection of geometric items which have been bounded by
// axis-aligned bounding boxes in N-dimensional Euclidean space.
...
struct bin_collection
{
    ...
    // Array of index shifts used for storing bin items (used to locate first
    // item in each within the item array). (u)
...
unsigned
coords_to_index(
    vector<N, unsigned> const& coords, vector<N, unsigned> const& grid_size)
{
...
// advances coords to the next coord for iterating through a range in a grid.
// returns true if there was a valid next coord
template<unsigned int N>
bool
go_to_next(
...
// advances backwards to the next coord for iterating through a range in a
// grid. returns true if there was a valid prev coord
template<unsigned int N>
bool
go_to_prev(
    vector<N, unsigned>& coords,
    ...
    bin_collection_item<ItemType, N, T> const& item)
{
    ...
    {
        clamped_starts[dim]
            = std::max(std::min(starts[dim], grid_size[dim] - 1), 0U);
        clamped_ends[dim]
            = std::max(std::min(ends[dim], grid_size[dim] - 1), 0U);
    }
    ...
    unsigned resolution_multiplier = 2)
{
    ...
    for (unsigned dim = 0; dim < N; ++dim)
    {
        grid_size[dim] = unsigned(
            size_sums[dim] > EPSILON ? std::ceil(
                double(resolution_multiplier) * double(num_items)
                * bounds.size[dim] / size_sums[dim])
                                     : 1);
    }
    ItemListList bins;
    unsigned num_bins = 1;
    ...
    {
        // start in each row or column or (...etc into more dimensions) of the
        // range that includes the item
        Coords starts;
        // end in each row or column or (...etc into more dimensions) of the
        // range that includes the item
        Coords ends;
        ...
        for (unsigned dim = 0; dim < N; ++dim)
        {
            ...
            starts[dim] = unsigned(
                grid_size[dim] * (item_min - mesh_min) / bounds.size[dim]);
            ends[dim] = unsigned(
                grid_size[dim] * (item_max - mesh_min) / bounds.size[dim]);
        }

        typename ItemListList::iterator bins_begin = bins.begin();
        astroid::fill(bins_begin, starts, ends, grid_size, iter->second);
    }

    return optimize_to_bin_collection<
        typename ItemListList::const_iterator,
        ItemType,
        N,
        T>(bins.begin(), grid_size, bounds);
}

template<class ItemType, unsigned int N, class T>
vector<N, unsigned>
get_coords_for_point(
    bin_collection<ItemType, N, T> const& bin_col, vector<N, T> const& point)
{
    typedef vector<N, T> Vector;
    typedef vector<N, unsigned> Coords;
    ...
    for (unsigned dim = 0; dim < N; ++dim)
    {
        coords[dim] = unsigned(
            ...
            * (point[dim] - bin_col.bounds.corner[dim])
            / bin_col.bounds.size[dim]);

        coords[dim] = astroid::clamp(coords[dim], 0U, bin_col.grid_size[dim]);
    }
    return coords;
}
...
// given the grid coordinates, get an iterator (just a pointer in this case)
// one past the last item in that bin
template<class ItemType, unsigned int N, class T>
ItemType const*
get_bin_end(
    bin_collection<ItemType, N, T> const& bin_col,
...
// in the positive direction or negative direction based on
// {positive_direction} calling {fn} on every item found in those bins adding
// up their results and storing it in {*sum} if not null if any of those calls
// to {fn} return false, this function returns false instead of setting {*sum}.
//
