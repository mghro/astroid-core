...
#include <astroid/geometry/multiple_source_view.hpp>
...
fit_view_to_scene(
    box<3, double> const& scene_bounds, multiple_source_view const& view)
{
    auto modelview = create_modelview(view);
    std::vector<vector3d> corners;
    corners.push_back(scene_bounds.corner);
    corners.push_back(
        scene_bounds.corner + make_vector(scene_bounds.size[0], 0., 0.));
    corners.push_back(
        scene_bounds.corner + make_vector(0., scene_bounds.size[1], 0.));
    corners.push_back(
        scene_bounds.corner + make_vector(0., 0., scene_bounds.size[2]));
    corners.push_back(
        scene_bounds.corner
        + make_vector(scene_bounds.size[0], scene_bounds.size[1], 0.));
    corners.push_back(
        scene_bounds.corner
        + make_vector(0., scene_bounds.size[1], scene_bounds.size[2]));
    corners.push_back(
        scene_bounds.corner
        + make_vector(scene_bounds.size[0], 0., scene_bounds.size[2]));
    ...
    v.display_surface = make_box(min, max - min);
    return v;
}

// determine, based on the multiple_source_view, whether we should preprocess
// verts for beams_eye_view or if normal OpenGl transformations are sufficient
bool
should_preprocess_verts(multiple_source_view const& view)
{
    return false; //(view.distance[0] != view.distance[1]);
}

// create a (row-major) orthographic projection matrix from a view
matrix<4, 4, double> static ortho_projection(multiple_source_view const& view)
{
    double right
        = view.display_surface.corner[0] + view.display_surface.size[0];
    ...
    return make_matrix<double>(
        2.0 / right_minus_left,
        0.0,
        0.0,
        -(right + left) / right_minus_left,
        0.0,
        2.0 / top_minus_bottom,
        0.0,
        -(top + bottom) / (top - bottom),
        0.0,
        0.0,
        -2.0 / clip_far_minus_near,
        -(CLIP_FAR + CLIP_NEAR) / clip_far_minus_near,
        0.0,
        0.0,
        0.0,
        1.0);
...
//    double right = scale * (view.display_surface.corner[0] +
//    view.display_surface.size[0]); double left = scale *
//    view.display_surface.corner[0]; double w = right - left; double top =
//    scale * (view.display_surface.corner[1] + view.display_surface.size[1]);
...
double
modelview_translation(multiple_source_view const& view)
{
    // if (should_preprocess_verts(view) || view.distance[0]==0)
    //{
    return 500.; // Should be ok with this value. Only effects clipping not
                 // scale or size
    //  }
    //  else
    //  {
    //// Limit of 10 here prevent possible issues with very small distances
    ///(which should never happen
    //      return std::max(10.0, std::max(view.distance[0],
    //      view.distance[1]));
    //  }
}

// does all the work for preprocess_point and preprocess_point_inverse
vector3d static _preprocess_point(
    multiple_source_view const& view, vector3d const& v, bool inverse = false)
{
    // if (should_preprocess_verts(view))
    {
        ...
        if (view.distance[0] != 0)
        {
            ...
            double scale = inverse ? (z_x / view.distance[0])
                                   : (view.distance[0] / z_x);
        ...
        if (view.distance[1] != 0)
        {
            ...
            double scale = inverse ? (z_y / view.distance[1])
                                   : (view.distance[1] / z_y);
        ...
        return (x * side) + (y * up) + (z * forward);
    }
    // else // either orthographic or the projection matrix can handle the
    // perspective for us.
    //{
    //    return v;
    //}
...
// preprocess a point to account for the effects of multiple virtual source
// points (or leaves it alone and uses normal OpenGL transforms instead)
vector3d
preprocess_point(multiple_source_view const& view, vector3d const& v)
{
...
vector3d
preprocess_point_inverse(multiple_source_view const& view, vector3d const& v)
{
    return _preprocess_point(view, v, true);
}

// add translation to a transformation matrix
void static translate(matrix<4, 4, double>& m, vector3d const& v)
{
...
astroid::multiple_source_view
pan_view(
    astroid::multiple_source_view const& view, vector<3, double> const& offset)
{
    astroid::multiple_source_view v = view;
...
astroid::multiple_source_view
center_on(
    astroid::multiple_source_view const& view, vector<3, double> const& center)
{
    astroid::multiple_source_view v = view;
    v.center = center;
    return v;
...
astroid::multiple_source_view
zoom_in(astroid::multiple_source_view const& view, double zoom_factor)
{
    astroid::multiple_source_view v = view;
    vector2d center
        = v.display_surface.corner + (0.5 * v.display_surface.size);
    v.display_surface.corner
        = ((v.display_surface.corner - center) / zoom_factor) + center;
    v.display_surface.size
        = ((v.display_surface.size - center) / zoom_factor) + center;
    return v;
}

// project a point from 3d world space to 2d coordinates in a plane containing
// view.center and lying perpendicular to view.direction
vector2d
project(vector3d const& v, multiple_source_view const& view)
{
    vector2d result;
...
// project a point from 2d coordinates in the plane containing view.center and
// lying perpendicular to view.direction3d to 3d world coordinates
vector3d
unproject(vector2d const& v, multiple_source_view const& view)
{
    vector3d view_direction = unit(view.direction);
    vector3d side = unit(cross(view_direction, view.up));
    vector3d up = unit(cross(side, view_direction));
...
// project view bounds to the plane containing view.center and lying
// perpendicular to view.direction3d
box<2, double>
make_2d_scene_box_from_view(
    vector3d const& origin, multiple_source_view const& view)
{
    vector2d offset = project(origin, view);
    return make_box(
        view.display_surface.corner - offset, view.display_surface.size);
}

// make a plane perpendicular to the view at some distance from the focus point
plane<double>
make_plane(double distance, multiple_source_view const& view)
{
    return plane<double>(
        view.center - distance * view.direction, view.direction);
}
