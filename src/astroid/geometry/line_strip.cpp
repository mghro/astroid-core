...
bool
attempt_to_append_strip(
    std::list<std::list<vector2d>>& strips,
    std::list<vector2d>& strip,
    vector2d const& p,
    double tolerance)
{
    for (std::list<std::list<vector2d>>::iterator i = strips.begin();
         i != strips.end();
         ++i)
    {
        if (&*i == &strip)
    ...
    std::vector<line_segment<2, double>> const& segments, double tolerance)
{
    ...
    for (std::list<std::list<vector2d>>::const_iterator i = strips.begin();
         i != strips.end();
         ++i, ++result_i)
    {
...
bool
is_polygon(std::vector<line_strip> const& strips, double tolerance)
{
    if (strips.size() != 1)
        return false;
    ...
    return strip.vertices.size() > 3
           && almost_equal(
               strip.vertices.front(), strip.vertices.back(), tolerance);
}

optional<polygon2>
as_polygon(std::vector<line_strip> const& strips, double tolerance)
{
...
as_polygon(
    optional<polygon2>* result,
    std::vector<line_strip> const& strips,
    double tolerance)
{
    if (strips.size() != 1)
    {
    ...
    *result = polygon2();
    copy(
        ...
        strip.vertices.end() - 1,
        allocate(&(**result).vertices, strip.vertices.size() - 1));
}

bool
is_polyset(std::vector<line_strip> const& strips, double tolerance)
{
    for (std::vector<line_strip>::const_iterator i = strips.begin();
         i != strips.end();
         ++i)
    {
        if (i->vertices.size() < 4
            || !almost_equal(
                i->vertices.front(), i->vertices.back(), tolerance))
        {
            return false;
        }
    }
...
optional<polyset>
as_polyset(std::vector<line_strip> const& strips, double tolerance)
{
