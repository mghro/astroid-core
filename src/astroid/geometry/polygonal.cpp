...
polygon2
make_polygon2(std::vector<vertex2> const& vertices)
{
    polygon2 poly;
...
double
get_area(polygon2 const& poly)
{
    return get_area_and_centroid(poly).first;
}

vector2d
get_centroid(polygon2 const& poly)
{
    return get_area_and_centroid(poly).second;
}

std::pair<double, vector2d>
get_area_and_centroid(polygon2 const& poly)
{
    double a = 0.0;
    double cx = 0.0;
    double cy = 0.0;
    ...
    return std::make_pair(
        std::fabs(0.5 * a), make_vector(cx / scale, cy / scale));
}

polygon2
scale(polygon2 const& poly, double factor)
{
...
struct point_scaling_fn
{
    ...
    vertex2
    operator()(vertex2 const& src) const
    {
...
polygon2
scale(polygon2 const& poly, vector<2, double> const& factor)
{
    point_scaling_fn fn;
    fn.factor = factor;
...
bool
is_inside(polygon2 const& poly, vector<2, double> const& p)
{
    bool c = false;
    for (polygon2_edge_view ev(poly); !ev.done(); ev.advance())
    {
...
bool
is_inside(polygon2 const& parent_poly, polygon2 const& child_poly)
{
...
void
create_polyset(polyset* set, polygon2 const& poly)
{
    *set = polyset();
...
polyset
make_polyset(polygon2 const& poly)
{
    polyset p;
...
void
add_hole(polyset& set, polygon2 const& hole)
{
...
polyset
remove_polyset_holes(polyset const& original_shape)
{
...
polyset
scale(polyset const& set, double factor)
{
    return scale(set, make_vector(factor, factor));
}
polyset
scale(polyset const& set, astroid::vector<2, double> const& factor)
{
    point_scaling_fn fn;
    fn.factor = factor;
...
std::pair<double, vector2d> static get_area_and_centroid(polyset const& set)
{
    double area = 0;
...
double
get_area(polyset const& set)
{
    return get_area_and_centroid(set).first;
}

vector2d
get_centroid(polyset const& set)
{
...
bool
is_inside(polyset const& set, vector<2, double> const& p)
{
    int inside_count = 0;
    for (auto const& i : set.polygons)
    {
...
double
distance_to_polyset(vector<2, double> const& p, polyset const& set)
{
    double d2p = 1.0e10;
    ...
    for (size_t k = 0; k < polygon_count; ++k)
    {
        ...
        for (size_t i = 0; i < vertex_count; ++i)
        {
            ...
            if (len >= 1.0e-8)
            {
                ...
                double spacing = 0.25;
                int split_count
                    = std::min(std::max(int(std::ceil(len / spacing)), 2), 30);
...
void
to_boost_polygon(boost_polygon_set& boost_set, polyset const& astroid_set)
{
...
void
from_boost_polygon(polygon2& poly, boost_polygon const& bp)
{
...
void
from_boost_polygon(polyset& astroid_set, boost_polygon_set const& boost_set)
{
    astroid_set = polyset();
    ...
    for (auto const& bp : boost_polys)
    {
        ...
        for (auto const& bh : bp.holes_)
        {
            ...
            astroid_set.holes.push_back(hole);
        }
    }
}

void
from_boost_polygon(std::vector<polygon2>& polys, boost_polygon_set const& set)
{
...
std::vector<polygon2>
as_polygon_list(polyset const& set)
{
...
bool
almost_equal(polyset const& set1, polyset const& set2, double tolerance)
{
    polyset xor_;
...
bool
almost_equal(polyset const& set1, polyset const& set2)
{
    polyset xor_;
    do_set_operation(&xor_, set_operation::XOR, set1, set2);
...
polyset
polyset_expansion(polyset const& src, double amount)
{
    polyset result;
    ...
    polyset* set, std::vector<polygon2> const& polygons)
{
    using namespace boost::polygon::operators;
    boost_polygon_set boost_set;
    for (std::vector<polygon2>::const_iterator i = polygons.begin();
         i != polygons.end();
         ++i)
    {
        boost_polygon bp;
...
bool
is_inside(structure_geometry_slice const& s, double p)
{
    return (
...
void static reset_structure_to_slice_list(
    structure_geometry* structure, slice_description_list const& slices)
{
...
structure_geometry static remove_empty_slices(
...
find_slice_at_exact_position(
    structure_polyset_list const& slices, double position, double thickness)
{
...
get_structure_slice(structure_geometry const& structure, double position)
{
    ...
    for (size_t i = 1; i < masters.size(); ++i)
    {
        if (p < masters[i].position)
        {
            ...
            {
                return some(find_slice_at_exact_position(
                    slices,
            ...
            {
                return some(find_slice_at_exact_position(
                    slices, masters[i].position, masters[i].thickness));
            }
        }
    }
    // Valid case when p is past the last slice position, but within its
    // thickness
    return some(find_slice_at_exact_position(
        slices, masters.rbegin()->position, masters.rbegin()->thickness));
}

std::vector<structure_geometry_slice>
get_structure_slices(
    structure_geometry const& structure, double p_low, double p_high)
{
    auto const& masters = structure.master_slice_list;
    auto const& slices = structure.slices;
    ...
    // Note: the initial value set below is critical to this functioning
    // properly
    ...
    for (size_t i = 1; i < masters.size(); ++i)
    {
        if (p_low < masters[i].position)
        {
            ...
            output.push_back(find_slice_at_exact_position(
                slices,
    ...
    for (size_t i = i_start + 1; i < masters.size(); ++i)
    {
        ...
        {
            if (p_high - masters[i - 1].position
                < masters[i].position - p_high)
            {
    ...
    // All slices have been added (but never reached p_high position), just
    // return
    return output;
}
...
std::pair<double, vector3d> static get_volume_and_centroid(
    structure_geometry const& structure)
{
    auto volume = 0.;
    ...
    for (; i != end_m; ++i)
    {
        ...
        {
            fold_in_below(
                volume, centroid, i_info, i->position, i->thickness * 0.5);
        }
        else
        {
            --temp;
...
double
get_volume(structure_geometry const& structure)
{
    return get_volume_and_centroid(structure).first;
}

vector3d
get_centroid(structure_geometry const& structure)
{
...
bool
is_inside(structure_geometry const& structure, vector<3, double> const& p)
{
    ...
    double tolerance)
{
    ...
    for (; i1 != end1; ++i1, ++i2)
    {
        ...
        auto r1 = find_slice_at_exact_position(
            slices1, i1->position, i1->thickness);
        auto r2 = find_slice_at_exact_position(
            slices2, i2->position, i2->thickness);
...
bool
almost_equal(structure_geometry const& a, structure_geometry const& b)
{
    ...
    structure_geometry const& structure2)
{
    ...
    {
        throw exception(
            "structure set_operation requires matching master lists");
    }
    ...
    {
        auto s1 = find_slice_at_exact_position(
            structure1.slices, i.position, i.thickness);
        auto s2 = find_slice_at_exact_position(
            structure2.slices, i.position, i.thickness);
...
structure_combination(
    set_operation op, std::vector<structure_geometry> const& structures)
{
    if (structures.size() < 2)
    {
        throw exception(
            "structure_combination requires at least two structures");
    }
...
bounding_box(polygon2 const& poly)
{
    ...
    return box ? *box : make_box(make_vector(0., 0.), make_vector(0., 0.));
}
void
compute_bounding_box(optional<box<2, double>>& box, polygon2 const& poly)
{
    vertex2_array::const_iterator i = poly.vertices.begin(),
    ...
    box = astroid::box<2, double>(min, max - min);
}

// for a polyset
box<2, double>
bounding_box(polyset const& region)
{
    optional<box<2, double>> box;
...
void
compute_bounding_box(optional<box<2, double>>& box, polyset const& region)
{
...
{
    optional<astroid::box<2, double>> xy_box;
    if (box)
    {
        xy_box = slice(*box, 2);
    }
    ...
    {
        auto slice = find_slice_at_exact_position(
            structure.slices, m_slice.position, m_slice.thickness);
    ...
    if (!structure.slices.empty() && xy_box)
    {
        ...
        vector<2, double> const& xy_corner = xy_box->corner;
        vector<2, double> const& xy_size = xy_box->size;

        box = astroid::box<3, double>(
    ...
    optional<box3d> const& sg_bounds)
{
    ...
    auto slices = get_structure_slices(
        sg, box.corner[structure_axis], get_high_corner(box)[structure_axis]);
    ...
    for (auto const& s : slices)
    {
        ...
        // Check center of voxel as this may be a fast "short-circuit" for many
        // cases
        ...
        else
        {
            ...
            // Note this isn't 100% accurate because holes can actually make a
            // polygon vertex outside and we don't catch that case
...
bool
almost_equal(polygon2 const& a, polygon2 const& b, double tolerance)
{
...
bool
almost_equal(polygon2 const& a, polygon2 const& b)
{
    polyset region_a, region_b, xor_region;
    create_polyset(&region_a, a);
    create_polyset(&region_b, b);
