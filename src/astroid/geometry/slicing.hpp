#ifndef ASTROID_GEOMETRY_SLICING_HPP
#define ASTROID_GEOMETRY_SLICING_HPP
...
bool
is_inside(slice_description const& s, double p);

// Is the point p inside one of the slices in the given list?
api(fun name(point_inside_slice_list))
bool
is_inside(slice_description_list const& slices, double p);

// Given a list of slices and a point p, get the slice position that's
// closest to p.
api(fun)
double
round_slice_position(slice_description_list const& slices, double p);
...
api(fun)
double
advance_slice_position(slice_description_list const& slices, double p, int n);
...
{
    return map(
        [](slice_description const& slice) { return slice.position; }, slices);
}
...
box1d
get_slice_list_bounds(slice_description_list const& slices);
