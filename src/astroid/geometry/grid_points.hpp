#ifndef ASTROID_GEOMETRY_GRID_POINTS_HPP
#define ASTROID_GEOMETRY_GRID_POINTS_HPP
...
class regular_grid_iterator : public boost::iterator_facade<
                                  ...
                                  boost::random_access_traversal_tag>
{
 ...
 private:
    ...
    bool
    equal(regular_grid_iterator const& other) const
    {
    ...
    void
    advance(typename regular_grid_iterator::difference_type n);
 ...
 public:
    ...
    begin() const
    {
        return const_iterator(
            &grid_, uniform_vector<N, unsigned>(0), grid_.p0);
    }
    const_iterator
    end() const
    {
        ...
        return const_iterator(
            &grid_, i, grid_.p0 + astroid::vector<N, T>(i) * grid_.spacing);
    }
...
template<unsigned N, typename T>
void
regular_grid_iterator<N, T>::increment()
{
...
template<unsigned N, typename T>
void
regular_grid_iterator<N, T>::decrement()
{
...
struct regular_grid_iterator_distance
{
    ...
    {
        static unsigned const I = N - I_;
        return (static_cast<Difference>(b[I]) - static_cast<Difference>(a[I]))
               + regular_grid_iterator_distance<
                     Difference,
                     N,
                     T,
                     N - (I + 1)>::apply(grid, a, b)
                     * grid.n_points[I];
    }
};
...
template<unsigned N, typename T>
auto
regular_grid_iterator<N, T>::distance_to(regular_grid_iterator const& other)
    const -> typename regular_grid_iterator::difference_type
{
    assert(*grid == *other.grid);
    return impl::regular_grid_iterator_distance<
        typename regular_grid_iterator<N, T>::difference_type,
        N,
        T,
        N>::apply(*grid, i, other.i);
}
