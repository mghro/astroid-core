#ifndef ASTROID_GEOMETRY_REGULAR_GRID_HPP
#define ASTROID_GEOMETRY_REGULAR_GRID_HPP
...
ASTROID_GEOMETRY_DEFINE_TYPEDEFS(regular_grid)
...
vector<N, T>
get_grid_point(
    regular_grid<N, T> const& grid, vector<N, unsigned> const& index)
{
    vector<N, T> p;
...
template<unsigned N, typename T>
vector<N, T>
get_center(regular_grid<N, T> const& grid)
{
    vector<N, T> p;
    for (unsigned i = 0; i < N; ++i)
...
template<unsigned N, typename T>
vector<N, T>
get_size(regular_grid<N, T> const& grid)
{
    vector<N, T> s;
    for (unsigned i = 0; i < N; ++i)
...
template<unsigned N, typename T>
regular_grid<N - 1, T>
slice(regular_grid<N, T> const& grid, unsigned axis)
{
...
unslice(
    ...
    unsigned axis,
    astroid::regular_grid<1, T> const& extra)
{
    regular_grid<N + 1, T> r;
    ...
    vector<N, T> const& spacing)
{
    ...
    grid->p0
        = get_center(box)
          - (astroid::vector<N, T>(grid->n_points) * spacing - spacing) / 2;
}
...
// make_grid_covering_box(b, c) creates a grid that covers the box b with point
// count c.
api(fun trivial with(N : 1, 2, 3, 4; T : double))
template<unsigned N, typename T>
regular_grid<N, T>
...
template<unsigned N, typename T>
box<N, T>
bounding_box(regular_grid<N, T> const& grid)
{
    return box<N, T>(
        grid.p0,
        grid.spacing
            * vector<N, T>(grid.n_points - uniform_vector<N, unsigned>(1)));
}
template<unsigned N, typename T>
void
compute_bounding_box(optional<box<N, T>>& box, regular_grid<N, T> const& grid)
{
    astroid::box<N, T> bb = bounding_box(grid);
