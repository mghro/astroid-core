#ifndef ASTROID_GEOMETRY_ANGLE_HPP
#define ASTROID_GEOMETRY_ANGLE_HPP
...
class angle
{
 public:
    ...
    angle(angle<T, OtherUnits> const& other)
        : angle_(
            impl::angle_conversion<T, OtherUnits, Units>::apply(other.get()))
    {
    }
...
template<typename T, class Units>
struct cradle::type_info_query<astroid::angle<T, Units>>
{
    static void
...
template<typename T, class Units>
void
to_dynamic(cradle::dynamic* v, angle<T, Units> x)
{
    using cradle::to_dynamic;
    to_dynamic(v, angle<T, degrees>(x).get());
}
template<typename T, class Units>
void
from_dynamic(angle<T, Units>* x, cradle::dynamic const& v)
{
    using cradle::to_dynamic;
    T n;
    from_dynamic(&n, v);
...
template<typename T, class Units>
std::ostream&
operator<<(std::ostream& ostream, const angle<T, Units>& a)
{
