...
load_mesh_from_obj(astroid::file_path const& path)
{
    ...
    if (!f)
    {
        throw new astroid::file_error(path, "unable to open OBJ file");
        return mesh;
    }
    ...
    while (std::getline(obj, line))
    {
        ...
        if (tokens.size())
        {
            if ("v" == tokens[0])
            {
                if (tokens.size() != 4)
                {
                    throw astroid::exception("Error in OBJ format");
                }
            ...
            else if ("f" == tokens[0])
            {
                if (tokens.size() != 4)
                {
                    throw new astroid::exception("Error in OBJ format");
                }
...
bounding_box(triangle_mesh const& mesh)
{
    ...
    return astroid::box<3, double>(mins, maxs - mins);
}
