#ifndef ASTROID_IMAGING_CHANNEL_HPP
#define ASTROID_IMAGING_CHANNEL_HPP
...
channel_multiply(uint8_t a, uint8_t b)
{
    uint32_t x = unsigned(a) * unsigned(b) + 128;
    return uint8_t((x + (x >> 8)) >> 8);
}
static inline float
channel_multiply(float a, float b)
{
    return a * b;
}
static inline double
channel_multiply(double a, double b)
{
    return a * b;
}
