...
#include <astroid/imaging/value_transforms.hpp>
...
struct variant_image_value_transformer
{
    ...
    {
        *dst = transform_image_values_typed(
            src, *transform, *transformed_units);
    }
};
...
{
    return as_variant(
        transform_variant_image_values(image, transform, transformed_units));
}
