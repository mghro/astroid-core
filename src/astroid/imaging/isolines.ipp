...
void
add_single_edge_connection_lines(
    ...
    vector2d const& to_outer_edge)
{
    ...
    for (std::size_t i = 1; i != n_pixels;
         ++i, p += p_step, pixel += pixel_step)
    {
        values[1] = double(*pixel);

        double t = (raw_level - values[0]) / (values[1] - values[0]);
        if (t >= 0 && t < 1)
        {
    ...
    double level)
{
    ...
    // top edge
    impl::add_single_edge_connection_lines(
        lines,
        img.size[0],
    ...
    double level)
{
    ...
    impl::close_edge(
        ...
        img.step[0],
        raw_level,
        get_pixel_center(img, make_vector<unsigned>(0, 0)) - y_vector * 0.5,
        x_vector);
    // bottom edge
    impl::close_edge(
        lines,
    ...
    impl::close_edge(
        ...
        img.step[1],
        raw_level,
        get_pixel_center(img, make_vector<unsigned>(0, 0)) - x_vector * 0.5,
        y_vector);
    // right edge
    impl::close_edge(
        lines,
