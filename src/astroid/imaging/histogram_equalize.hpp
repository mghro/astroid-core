#ifndef ASTROID_IMAGING_HISTOGRAM_EQUALIZE_HPP
#define ASTROID_IMAGING_HISTOGRAM_EQUALIZE_HPP
...
void
histogram_equalize(
    image<N, DstT, DstSP> const& dst, image<N, SrcT, SrcSP> const& src)
{
...
void
create_histogram_equalization_palette(
    image<N, SrcT, SrcSP> const& img, PaletteT* palette)
{
    ...
    if (product(img.size) > 10000)
    {
        compute_raw_histogram(
            &hist, subsampled_view(img, make_vector<unsigned>(4, 4)));
    }
    else
    ...
    for (int i = min_channel_v; i <= max_channel_v; ++i)
    {
        ...
        palette[index] = PaletteT(
            std::min(tmp, int(std::numeric_limits<PaletteT>::max())));
    }
}
