#ifndef ASTROID_IMAGING_GEOMETRY_HPP
#define ASTROID_IMAGING_GEOMETRY_HPP
...
get_spatial_mapping(untyped_image_base<N> const& img)
{
    ...
    rotation.set_column(N, unslice(uniform_vector<N, double>(0), N, 1));
    return translation(vector<N, double>(img.origin)) * rotation;
}
...
void
set_spatial_mapping(
    untyped_image_base<N>& img, regular_grid<N, double> const& grid)
{
...
template<unsigned N>
bool
is_axis_aligned(untyped_image_base<N> const& img)
{
    for (unsigned i = 0; i != N; ++i)
    {
        if (img.axes[i][i] <= 0)
...
template<unsigned N>
bool
is_orthogonal_to_axes(untyped_image_base<N> const& img)
{
    for (unsigned i = 0; i != N; ++i)
    {
        unsigned nonzeros = 0;
...
vector<N, double>
get_pixel_center(
    untyped_image_base<N> const& img, vector<N, unsigned> const& p)
{
    return transform_point(
        get_spatial_mapping(img),
        vector<N, double>(p) + uniform_vector<N, double>(0.5));
}

template<unsigned N>
vector<N, double>
get_origin(untyped_image_base<N> const& img)
{
    return img.origin;
}
...
template<unsigned N>
vector<N, double>
get_spacing(untyped_image_base<N> const& img)
{
...
get_grid(untyped_image_base<N> const& img)
{
...
void
create_image_on_grid(
    image<N, Pixel, unique>& img, regular_grid<N, double> const& grid)
{
...
template<unsigned N, class Pixel>
image<N, Pixel, view>
make_view_on_grid(Pixel* pixels, regular_grid<N, double> const& grid)
{
    image<N, Pixel, view> img(
        pixels, grid.n_points, get_contiguous_steps(grid.n_points));
    set_spatial_mapping(img, grid);
    return img;
}
...
make_const_view_on_grid(Pixel* pixels, regular_grid<N, double> const& grid)
{
    image<N, Pixel, const_view> img(
        pixels, grid.n_points, get_contiguous_steps(grid.n_points));
    set_spatial_mapping(img, grid);
    return img;
}
