#ifndef ASTROID_IMAGING_INTEGRAL_HPP
#define ASTROID_IMAGING_INTEGRAL_HPP
...
class image_integral_computer
{
 public:
    ...
    double
    compute_integral_to(double distance);

    // Get the integral from the ray origin to the given point, projected onto
    // the ray.
    double
    compute_integral_to(vector<N, double> const& p);
    ...
    double
    compute_distance_to(double integral);

    // Get the intersection between the ray and the image volume.
    ray_box_intersection<N, double> const&
    get_intersection()
    {
...
#include <astroid/imaging/integral.ipp>
