    ...
    line_segment<N, double> const& segment)
{
    ...
    image_integral_computer<N, Pixel, Storage> computer(
        img,
        ray,
...
struct line_segment_image_integral_computer
{
    ...
    void
    operator()(image<N, Pixel, Storage> const& img)
    {
        result = compute_image_integral_over_line_segment_min_max(
            img, segment, min, max, zero_value);
    }
};
} // namespace impl
    ...
    : img_(img), ray_(ray), min_(min), max_(max), zero_value_(zero_value)
{
    ...
    vector<N, double> slope
        = transform_vector(inverse_spatial_mapping, ray.direction);
    ...
    vector<N, double> start
        = origin_in_image + slope * intersection_.entrance_distance;
    ...
    for (unsigned i = 0; i < N; ++i)
    {
        ...
        {
            next_grid_line_[i]
                = (std::floor(start[i]) + 1 - origin_in_image[i])
                  * reciprocal_slope_[i];
        }
        else
        {
...
image_integral_computer<N, Pixel, SP>::compute_integral_to(double distance)
{
    ...
    {
        extra_integral
            = (distance - intersection_.exit_distance) * zero_value_;
    ...
    else
    {
        ...
        vector<N, double> next_grid_line = next_grid_line_ - reciprocal_slope_;
...
template<unsigned N, class Pixel, class SP>
double
image_integral_computer<N, Pixel, SP>::compute_integral_to(
    vector<N, double> const& p)
{
    return compute_integral_to(dot(p - ray_.origin, ray_.direction));
}

template<unsigned N, class Pixel, class SP>
double
image_integral_computer<N, Pixel, SP>::compute_distance_to(double integral)
{
