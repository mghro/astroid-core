    ...
    double v2)
{
    recorder.tris->push_back(colored_triangle<2, double>(
        ...
        colored_vertex<2, double>(
            p1, interpolate(recorder.low_color, recorder.high_color, v1)),
        colored_vertex<2, double>(
            p2, interpolate(recorder.low_color, recorder.high_color, v2))));
}
...
add_corner_tris(
    ...
    vector2d const& to_edge0,
    vector2d const& to_edge1)
{
    ...
    if (raw_value >= raw_low && raw_value < raw_high)
    {
        ...
        record_triangle(
            recorder, p, v, p + to_edge0, v, p + to_edge0 + to_edge1, v);
        record_triangle(
            recorder, p, v, p + to_edge0 + to_edge1, v, p + to_edge1, v);
    }
}
    ...
    vector2d const& to_edge)
{
    ...
    for (std::size_t i = 1; i != n_pixels;
         ++i, p += p_step, pixel += pixel_step, values[0] = values[1])
    {
        ...
        double t_low
            = clamp((raw_low - values[0]) / (values[1] - values[0]), 0., 1.);
        double t_high
            = clamp((raw_high - values[0]) / (values[1] - values[0]), 0., 1.);
        ...
        if (t_low != t_high)
        {
            ...
            double v0 = (interpolate(values[0], values[1], t0) - raw_low)
                        / (raw_high - raw_low);
            double v1 = (interpolate(values[0], values[1], t1) - raw_low)
                        / (raw_high - raw_low);
            // Record the quad.
            add_edge_quad(
                recorder, p + t0 * p_step, v0, p + t1 * p_step, v1, to_edge);
        }
    }
}
    ...
    double high)
{
    ...
    vector2d x_vector
        = transform_vector(spatial_mapping, make_vector<double>(1, 0));
    vector2d y_vector
        = transform_vector(spatial_mapping, make_vector<double>(0, 1));

    // top edge
    impl::add_single_edge_triangles(
        recorder,
        img.size[0],
    ...
    impl::add_single_edge_triangles(
        recorder,
        img.size[1],
        get_pixel_iterator(img, make_vector<unsigned>(img.size[0] - 1, 0)),
        img.step[1],
        raw_low,
        raw_high,
        get_pixel_center(img, make_vector<unsigned>(img.size[0] - 1, 0)),
        y_vector,
        x_vector * 0.5);

    // four corners
    impl::add_corner_tris(
        recorder,
        img,
        raw_low,
        raw_high,
...
struct add_isoband_triangles_fn
{
    ...
    void
    operator()(image<2, Pixel, Storage> const& img)
    {
...
add_isoband_triangles(
    Recorder& recorder,
    image<2, variant, Storage> const& img,
    double low,
    double high)
{
