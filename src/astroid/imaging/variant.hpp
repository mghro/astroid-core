...
#include <astroid/imaging/iterator.hpp>
...
{
    static astroid::pixel_format const value = pixel_format::GRAY;
};
template<class Channel>
struct pixel_format_of_type<rgb<Channel>>
{
    static astroid::pixel_format const value = pixel_format::RGB;
};
template<class Channel>
struct pixel_format_of_type<rgba<Channel>>
{
    static astroid::pixel_format const value = pixel_format::RGBA;
};
...
template<class Channel>
struct channel_type_of_type<rgb<Channel>> : channel_type_of_type<Channel>
{
};
template<class Channel>
struct channel_type_of_type<rgba<Channel>> : channel_type_of_type<Channel>
{
};
...
// type.
class image_type_mismatch : public astroid::exception
{
 public:
    image_type_mismatch(
    ...
    pixel_format
    get_expected_format() const
    {
        return expected_format_;
    }
    channel_type
    get_expected_type() const
    {
    ...
    pixel_format
    get_actual_format() const
    {
        return actual_format_;
    }
    channel_type
    get_actual_type() const
    {
        return actual_type_;
    }
    string const&
 ...
 private:
    ...
    std::shared_ptr<string> expected_;

    channel_type actual_type_;
    pixel_format actual_format_;
    std::shared_ptr<string> actual_;
};

template<class Pixel>
void
set_type_info(variant_type_info& info)
{
...
template<class Pixel>
void
match_type_info(variant_type_info const& info)
{
...
void
dispatch_variant(variant_type_info const& info, Functor& f)
{
    switch (info.format)
    {
...
template<class Functor>
void
dispatch_gray_variant(variant_type_info const& info, Functor& f)
{
...
static inline void
swap(variant_view_pointer& a, variant_view_pointer& b)
{
    using std::swap;
    swap(a.view, b.view);
...
static inline bool
operator==(variant_view_pointer const& a, variant_view_pointer const& b)
{
...
template<class Pixel>
void
cast_pointer(variant_view_pointer& dst, Pixel* src)
{
    dst.view = src;
...
template<class Pixel>
void
cast_pointer(Pixel*& dst, variant_view_pointer const& src)
{
...
template<class Pixel>
void
cast_pointer(variant_view_pointer& dst, unique_pointer<Pixel> const& src)
{
    dst.view = src.ptr;
    set_type_info<Pixel>(dst.type_info);
}
...
static inline void
swap(variant_const_view_pointer& a, variant_const_view_pointer& b)
{
    using std::swap;
    swap(a.view, b.view);
    swap(a.type_info, b.type_info);
...
static inline bool
operator==(
    variant_const_view_pointer const& a, variant_const_view_pointer const& b)
{
    return a.view == b.view && a.type_info == b.type_info;
}
static inline bool
...
template<class Pixel>
void
cast_pointer(variant_const_view_pointer& dst, Pixel const* src)
{
    dst.view = src;
    set_type_info<Pixel>(dst.type_info);
}
template<class Pixel>
void
cast_pointer(Pixel const*& dst, variant_const_view_pointer const& src)
{
    match_type_info<Pixel>(src.type_info);
...
template<class Pixel>
void
cast_pointer(variant_const_view_pointer& dst, shared_pointer<Pixel> const& src)
{
    dst.view = src.view;
    set_type_info<Pixel>(dst.type_info);
}
static inline void
cast_pointer(variant_const_view_pointer& dst, variant_view_pointer const& src)
{
    dst.view = src.view;
...
template<class Pixel>
void
cast_pointer(variant_const_view_pointer& dst, unique_pointer<Pixel> const& src)
{
    dst.view = src.ptr;
    set_type_info<Pixel>(dst.type_info);
}
...
static inline bool
operator==(variant_shared_pointer const& a, variant_shared_pointer const& b)
{
    return a.view == b.view && a.type_info == b.type_info;
}
static inline bool
...
static inline void
swap(variant_shared_pointer& a, variant_shared_pointer& b)
{
    using std::swap;
...
template<class Pixel>
void
cast_pointer(variant_shared_pointer& dst, shared_pointer<Pixel> const& src)
{
...
template<class Pixel>
void
cast_pointer(shared_pointer<Pixel>& dst, variant_shared_pointer const& src)
{
    match_type_info<Pixel>(src.type_info);
    dst.ownership = src.ownership;
...
static inline void
cast_pointer(
    variant_const_view_pointer& dst, variant_shared_pointer const& src)
{
    dst.type_info = src.type_info;
    dst.view = src.view;
}
template<class Pixel>
void
cast_pointer(Pixel const*& dst, variant_shared_pointer const& src)
{
...
image<N, variant, Storage>
as_variant(image<N, Pixel, Storage> const& img)
{
...
cast_variant(image<N, variant, Storage> const& img)
{
...
void
apply_fn_to_variant(Functor& f, image<N, variant, Storage> const& img)
{
...
template<class Functor, unsigned N, class Storage>
void
apply_fn_to_gray_variant(Functor& f, image<N, variant, Storage> const& img)
{
    variant_cast_applier<N, Storage, Functor> applier;
    applier.img = &img;
    applier.f = &f;
...
ASTROID_DECLARE_REGULAR_IMAGE_INTERFACE(1, astroid::variant)
ASTROID_DECLARE_REGULAR_IMAGE_INTERFACE(2, astroid::variant)
ASTROID_DECLARE_REGULAR_IMAGE_INTERFACE(3, astroid::variant)
    ...
    int k)
{
    ...
    else
    {
        ...
        for (unsigned j = 0; j < images.size(); ++j)
        {
            ...
            {
                p_i[n] = int(std::floor(
                    (pos[n] - images.at(j).origin[n])
                    / (images.at(j).axes[n][n])));
                if (p_i[n] < 0)
                {
                    double dd = images.at(j).origin[n] - pos[n]
                                + 0.5 * images.at(j).axes[n][n];
                    d += dd * dd;
                    p_i[n] = 0;
                }
                else if (p_i[n] >= images.at(j).size[n])
                {
                    double dd
                        = pos[n]
                          - (images.at(j).origin[n]
                             + images.at(j).axes[n][n] * images.at(j).size[n])
                          + 0.5 * images.at(j).axes[n][n];
                    d += dd * dd;
            ...
            switch (N)
            {
                ...
                case 3:
                    index
                        = p_i[2] * images.at(j).size[1] * images.at(j).size[0]
                          + p_i[1] * images.at(j).size[0] + p_i[0];
                    break;
            }
            ...
            {
                min_dist = 0.0;
                img.pixels.ptr[i] += double(
                    image_const_view.pixels[index]
                        * images.at(j).value_mapping.slope
                    + images.at(j).value_mapping.intercept);
            }
...
combine_images(
    // List of images to combine. Each image should ideally be positioned
    // adjacent to the others. The resulting image size will be the total
    // bounding box of the images in the provided list. If the provided images
    // have overlapping pixels, the resulting pixel values for the combined
    // image will be a sum of the overlapping image pixel values. If there is a
    // gap between the provided images or a gap between the combined bounding
    // box and the provided images, the zero value pixels will be filled in by
    // the next closest pixel's value.
    std::vector<image<N, double, shared>> const& images)
{
    image<N, double, unique> img;
    ...
    for (unsigned j = 1; j < images.size(); ++j)
    {
        for (unsigned i = 0; i < N; ++i)
        {
            ...
            double temp_end = images.at(j).origin[i]
                              + images.at(j).axes[i][i] * images.at(j).size[i];
            if (temp_end > end[i])
            {
                end[i] = temp_end;
            }
...
// Float image
image<N, double, shared>
convert_float_to_double_image(image<N, variant, shared> const& img)
{
    image<N, double, unique> img_d;
    create_image_on_grid(img_d, get_grid(img));
    set_value_mapping(
        img_d,
        img.value_mapping.intercept,
        img.value_mapping.slope,
        img.units);
