#ifndef ASTROID_IMAGING_UTILITIES_HPP
#define ASTROID_IMAGING_UTILITIES_HPP
...
void
copy_pixels(image<N, Pixel1, SP1> const& dst, image<N, Pixel2, SP2> const& src)
{
    copy_pixel_fn fn;
...
template<unsigned N, class Pixel1, class SP1, class Pixel2, class SP2>
void
copy_image(image<N, Pixel1, SP1>& dst, image<N, Pixel2, SP2> const& src)
{
    copy_pixels(dst, src);
...
struct copy_value_mapped_pixel_fn
{
    ...
    template<class Src>
    void
    operator()(double& dst, Src const& src)
    {
...
void
copy_value_mapped_pixels(
    image<N, double, SP1> const& dst, image<N, Pixel2, SP2> const& src)
{
...
struct variant_image_copier
{
    ...
    void
    operator()(image<N, Pixel, SrcSP> const& src)
    {
        image<N, Pixel, unique> tmp;
        create_image(tmp, src.size);
        copy_image(tmp, src);
...
void
fill_pixels(image<N, Pixel, SP> const& dst, Pixel value)
{
