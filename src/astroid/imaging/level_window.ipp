...
struct level_window_pixel_fn
{
    ...
    template<class Src>
    void
    operator()(uint8_t& dst, Src const& src)
    {
...
template<unsigned N, class Pixel, class SP>
image<N, uint8_t, shared>
apply_level_window(image<N, Pixel, SP> const& src, double level, double window)
{
    linear_function<double> inverse_value_mapping = inverse(src.value_mapping);
    return apply_raw_level_window(
        src,
...
struct apply_level_window_fn
{
    ...
    void
    operator()(image<N, Pixel, SP> const& src)
    {
...
void
create_level_window_palette(PaletteT* palette, double level, double window)
{
...
void
apply_paletted_raw_level_window(
    image<N, DstT, DstSP> const& dst,
    image<N, SrcT, SrcSP> const& src,
    double level,
    double window)
{
    // Create the appropriate palette.
    std::vector<DstT> palette(
        int((std::numeric_limits<
             typename unsigned_channel_type<SrcT>::type>::max)())
        + 1);
