...
struct image : untyped_image_base<N>
{
    ...
    typedef
        typename Storage::template pointer_type<N, Pixel>::type pointer_type;
    typedef
        typename Storage::template iterator_type<N, Pixel>::type iterator_type;
    ...
    image(
        ...
        astroid::units const& units = no_units)
    {
        this->pixels = pixels;
        this->size = size;
        this->step = step;
...
template<unsigned N, class Pixel, class Storage>
void
swap(image<N, Pixel, Storage>& a, image<N, Pixel, Storage>& b)
{
    using std::swap;
...
template<unsigned N>
void
copy_untyped_image_info(
    untyped_image_base<N>& dst, untyped_image_base<N> const& src)
{
    dst.size = src.size;
...
template<unsigned N>
bool
empty(untyped_image_base<N> const& img)
{
...
template<unsigned N>
vector<N, ptrdiff_t>
get_contiguous_steps(vector<N, unsigned> const& size)
{
...
template<unsigned N, class Pixel, class Storage>
void
swap_axes(image<N, Pixel, Storage>& img, unsigned a, unsigned b)
{
    using std::swap;
    if (a == b)
        return;
...
bool
same_pixel_data(
    image<N, Pixel, Storage> const& a, image<N, Pixel, Storage> const& b)
{
...
template<unsigned N>
void
reset_spatial_mapping(untyped_image_base<N>& img)
{
...
template<unsigned N>
void
reset_value_mapping(untyped_image_base<N>& img)
{
...
void
copy_value_mapping(
    untyped_image_base<N1>& dst, untyped_image_base<N2> const& src)
{
    dst.value_mapping = src.value_mapping;
    dst.units = src.units;
}
...
template<unsigned N>
bool
same_spatial_mapping(
    untyped_image_base<N> const& a, untyped_image_base<N> const& b)
{
    for (unsigned i = 0; i != N; ++i)
    {
...
bool
same_value_mapping(
    untyped_image_base<N1> const& a, untyped_image_base<N2> const& b)
{
...
template<class Step>
inline void
cast_step(Step& dst, Step const& src)
{
    dst = src;
}
...
inline void
cast_pointer(Pointer& dst, Pointer const& src)
{
    dst = src;
}
...
DstImage
cast_image(image<N, SrcPixel, SrcStorage> const& img)
{
    DstImage r;
...
image<N, Pixel, ToStorage>
cast_storage_type(image<N, Pixel, FromStorage> const& img)
{
...
template<unsigned N, class Pixel, class Storage>
bool
is_lazy(image<N, Pixel, Storage> const& img)
{
    return false;
}
...
double
apply_linear_function(linear_function<double> const& f, Pixel x)
{
    return apply(f, x);
}
...
template<unsigned N, class Pixel>
image<N, Pixel, view>
make_view(Pixel* pixels, vector<N, unsigned> const& size)
{
...
image<N, Pixel, view>
as_view(image<N, Pixel, SP> const& img)
{
...
template<class Pixel>
inline Pixel const*
get_iterator(Pixel const* ptr)
{
    return ptr;
}
template<class Pixel>
inline void
cast_pointer(Pixel const*& dst, Pixel* src)
{
    dst = src;
}
...
make_const_view(Pixel const* pixels, vector<N, unsigned> const& size)
{
    return image<N, Pixel, const_view>(
        pixels, size, get_contiguous_steps(size));
}
...
template<unsigned N, class Pixel, class SP>
image<N, Pixel, const_view>
as_const_view(image<N, Pixel, SP> const& img)
{
...
template<class Pixel>
inline void
swap(shared_pointer<Pixel>& a, shared_pointer<Pixel>& b)
{
    using std::swap;
...
template<class Pixel>
inline bool
operator==(shared_pointer<Pixel> const& a, shared_pointer<Pixel> const& b)
{
...
template<class Pixel>
inline shared_pointer<Pixel>
operator+(shared_pointer<Pixel> const& ptr, ptrdiff_t step)
{
...
template<class Pixel>
inline shared_pointer<Pixel>&
operator+=(shared_pointer<Pixel>& ptr, ptrdiff_t step)
{
    ptr.view += step;
    return ptr;
}
...
template<class Pixel>
inline Pixel const*
get_iterator(shared_pointer<Pixel> const& ptr)
{
    return ptr.view;
}
...
template<class Pixel>
inline void
cast_pointer(Pixel const*& dst, shared_pointer<Pixel> const& src)
{
    dst = src.view;
}
template<class Pixel>
inline void
cast_pointer(shared_pointer<Pixel>& dst, Pixel const* src)
{
    dst.ownership.reset();
    dst.view = src;
}
...
template<class Pixel>
void
initialize_shared_pixel_pointer(shared_pointer<Pixel>& ptr, Pixel* pixels)
{
    ptr.ownership
        = make_unique_ptr_owner<Pixel[]>(std::unique_ptr<Pixel[]>(pixels));
    ptr.view = pixels;
}
...
template<class Pixel>
inline void
swap(unique_pointer<Pixel>& a, unique_pointer<Pixel>& b)
{
    using std::swap;
    swap(a.ptr, b.ptr);
};
...
template<class Pixel>
inline bool
operator==(unique_pointer<Pixel> const& a, unique_pointer<Pixel> const& b)
{
    return a.ptr == b.ptr;
}
template<class Pixel>
...
template<class Pixel>
inline Pixel*
get_iterator(unique_pointer<Pixel> const& ptr)
{
    return ptr.ptr;
}
...
template<class Pixel>
inline void
cast_pointer(Pixel*& dst, unique_pointer<Pixel> const& src)
{
    dst = src.ptr;
}
template<class Pixel>
inline void
cast_pointer(Pixel const*& dst, unique_pointer<Pixel> const& src)
{
    dst = src.ptr;
}
...
template<unsigned N, class Pixel>
void
create_image(image<N, Pixel, unique>& img, vector<N, unsigned> const& size)
{
...
#define ASTROID_DECLARE_REGULAR_IMAGE_INTERFACE(N, T)                         \
    bool operator==(                                                          \
    ...
    template<>                                                                \
    struct cradle::type_info_query<astroid::image<N, T, astroid::shared>>     \
    {                                                                         \
        static void                                                           \
        get(api_type_info* info)                                              \
        {                                                                     \
            *info = make_api_type_info_with_named_type(                       \
                api_named_type_reference{                                     \
                    string("dosimetry"), string("image_" #N "d")});           \
        }                                                                     \
    };                                                                        \
    template<>                                                                \
...
#define ASTROID_DECLARE_REGULAR_IMAGE_INTERFACE_FOR_TYPE(T)                   \
    ...
    ASTROID_DECLARE_REGULAR_IMAGE_INTERFACE(2, T)                             \
    ASTROID_DECLARE_REGULAR_IMAGE_INTERFACE(3, T)

ASTROID_DECLARE_REGULAR_IMAGE_INTERFACE_FOR_TYPE(astroid::int8_t)
ASTROID_DECLARE_REGULAR_IMAGE_INTERFACE_FOR_TYPE(astroid::uint8_t)
ASTROID_DECLARE_REGULAR_IMAGE_INTERFACE_FOR_TYPE(astroid::int16_t)
ASTROID_DECLARE_REGULAR_IMAGE_INTERFACE_FOR_TYPE(astroid::uint16_t)
ASTROID_DECLARE_REGULAR_IMAGE_INTERFACE_FOR_TYPE(astroid::int32_t)
ASTROID_DECLARE_REGULAR_IMAGE_INTERFACE_FOR_TYPE(astroid::uint32_t)
ASTROID_DECLARE_REGULAR_IMAGE_INTERFACE_FOR_TYPE(astroid::int64_t)
ASTROID_DECLARE_REGULAR_IMAGE_INTERFACE_FOR_TYPE(astroid::uint64_t)
ASTROID_DECLARE_REGULAR_IMAGE_INTERFACE_FOR_TYPE(float)
ASTROID_DECLARE_REGULAR_IMAGE_INTERFACE_FOR_TYPE(double)

ASTROID_DECLARE_REGULAR_IMAGE_INTERFACE_FOR_TYPE(astroid::rgba8)

// Register the regular image types with the given API.
struct api_implementation;
void
register_image_types(api_implementation& api);
