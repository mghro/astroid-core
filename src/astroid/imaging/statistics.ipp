...
compute_image_min_max(image<N, T, SP> const& img)
{
    ...
    if (raw_min_max)
    {
        return min_max<double>(
            apply(img.value_mapping, raw_min_max->min),
            apply(img.value_mapping, raw_min_max->max));
    }
    else
        return none;
}
...
merge_min_max_values(std::vector<optional<min_max<T>>> const& values)
{
    ...
    for (auto const& i : values)
    {
        if (i)
        {
            ...
            {
                merged = min_max<T>(
                    std::min(i->min, merged->min),
                    std::max(i->max, merged->max));
            }
            else
                merged = i;
        }
...
image_list_min_max(std::vector<image<N, T, SP>> const& imgs)
{
    return merge_min_max_values(map(
        [](image<N, T, SP> const& img) { return image_min_max(img); }, imgs));
}
...
template<unsigned N, class T, class SP>
statistics<T>
raw_image_statistics(image<N, T, SP> const& img)
{
...
template<unsigned N, class T, class SP>
statistics<double>
compute_image_statistics(image<N, T, SP> const& img)
{
    size_t n_samples = product(img.size);
    if (n_samples != 0)
    {
...
template<unsigned N, class SP>
statistics<double>
compute_image_statistics(image<N, variant, SP> const& img)
{
...
template<unsigned N, class T, class SP>
statistics<double>
image_statistics(image<N, T, SP> const& img)
{
    ...
    image<N, T, SP> const& img, std::vector<size_t> const& indices)
{
    if (!indices.empty())
    {
        ...
        for (auto index : indices)
        {
            ...
            T x = *(pixels + index);
            if (!img_min || x < *img_min)
            {
                img_min = x;
            }
            if (!img_max || x > *img_max)
            {
                img_max = x;
        ...
        return statistics<double>(
            img_min ? optional<double>(apply(img.value_mapping, *img_min))
                    : none,
            img_max ? optional<double>(apply(img.value_mapping, *img_max))
                    : none,
    ...
    std::vector<weighted_grid_index> const& indices)
{
    if (!indices.empty())
    {
        ...
        for (auto const& index : indices)
        {
            ...
            double x
                = double(apply(img.value_mapping, *(pixels + index.index)));
            if (!img_min || x < *img_min)
            {
                img_min = x;
            }
...
merge_statistics(std::vector<statistics<T>> const& stats)
{
    ...
    {
        if (!merged_min || i.min && *i.min < *merged_min)
        {
            merged_min = i.min;
        }
        if (!merged_max || i.max && *i.max > *merged_max)
        {
            merged_max = i.max;
...
image_list_statistics(std::vector<image<N, T, SP>> const& imgs)
{
    return merge_statistics(
        map([](image<N, T, SP> const& img) { return image_statistics(img); },
            imgs));
}
        ...
        if (weight != 0)
        {
            total_variance
                += double(weight)
                   * square(
                       transform_point(
                           spatial_mapping, make_vector<double>(i))[0]
                       - *stats.mean);
    ...
    image<1, Bin, shared> const& histogram, statistics<double> const& stats)
{
    ...
    return variance ? some(sqrt(*variance)) : none;
}
...
struct standard_deviation_from_image_stats_fn
{
    ...
    void
    operator()(image<1, Pixel, shared> const& histogram)
    {
        this->result
...
optional<double> static standard_deviation_from_image_stats(
    image1 const& histogram, statistics<double> const& stats)
{
