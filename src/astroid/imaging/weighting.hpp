#ifndef ASTROID_IMAGING_WEIGHTING_HPP
#define ASTROID_IMAGING_WEIGHTING_HPP
...
{
    astroid::image<N, Pixel, SP> image;
    Weight weight;
};
...
struct weighted_combination_iterator
{
    ...
    typename replace_channel_type<Pixel, Weight>::type
    operator*()
    {
        ...
        for (size_t i = 0; i != n_pointers; ++i)
        {
            weighted_image_pointer<N, Weight, Pixel, SP> const& ptr
                = pointers[i];
...
template<unsigned N, class Weight, class Pixel, class SP>
weighted_combination_iterator<N, Weight, Pixel, SP>
get_iterator(weighted_combination_pointer<N, Weight, Pixel, SP> const& ptr)
{
    weighted_combination_iterator<N, Weight, Pixel, SP> i;
...
struct weighted_combination
{
    ...
    struct pointer_type
    {
        typedef weighted_combination_pointer<
            N,
            typename pixel_channel_type<WeightedPixel>::type,
            Pixel,
            SP>
            type;
    };
    ...
    {
        typedef weighted_combination_iterator<
            N,
            typename pixel_channel_type<WeightedPixel>::type,
            Pixel,
            SP>
    ...
    {
        typedef typename replace_channel_type<
            Pixel,
            typename pixel_channel_type<WeightedPixel>::type>::type type;
    };
    template<unsigned N, class WeightedPixel>
    struct step_type : SP::template step_type<N, Pixel>
    {
    };
};
...
make_weighted_combination(
    weighted_image<N, Weight, Pixel, SP> const* images, size_t n_images)
{
    if (n_images == 0)
    ...
    for (size_t i = 1; i != n_images; ++i)
    {
        ...
        if (img.size != img0.size || !same_value_mapping(img, img0)
            || !same_spatial_mapping(img, img0) || img.step != img0.step)
        {
            throw exception("weighted combination images aren't compatible");
        }
    }
