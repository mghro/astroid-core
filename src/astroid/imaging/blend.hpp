#ifndef ASTROID_IMAGING_BLEND_HPP
#define ASTROID_IMAGING_BLEND_HPP
...
struct raw_image_blend_fn
{
    ...
    void
    operator()(DstT& dst, SrcT1 const& src1, SrcT2 const& src2)
    {
...
raw_blend_images(
    ...
    image<N, SrcT1, SrcSP1> const& src1,
    image<N, SrcT2, SrcSP2> const& src2,
    double factor1,
    double factor2)
{
...
image<N, Pixel, shared>
blend_images(
    std::vector<image<N, Pixel, SP1>> const& imgs, std::vector<double> factors)
{
...
struct blend_images_fn
{
    ...
    void
    operator()(image<N, Pixel, SP1> const& img1)
    {
        dst = as_variant(
            blend_images(img1, cast_variant<Pixel>(img2), factor1, factor2));
    }
};
} // namespace impl
