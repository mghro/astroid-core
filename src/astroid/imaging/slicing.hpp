#ifndef ASTROID_IMAGING_SLICING_HPP
#define ASTROID_IMAGING_SLICING_HPP
...
decode_slice_position(
    ...
    untyped_image_base<N> const& img,
    unsigned axis,
    double position)
{
...
void
copy_slice_properties(
    image_slice<N, Pixel1, SP1>& dst, image_slice<N, Pixel2, SP2> const& src)
{
    dst.axis = src.axis;
...
Pointer
slice_pointer(Pointer const& ptr)
{
    return ptr;
}
...
sliced_view(image<N, T, SP> const& img, unsigned axis, int at)
{
    ...
    r.position = get_origin(img)[axis] + get_spacing(img)[axis] * (at + 0.5);
...
sliced_view(image<N, variant, SP> const& img, unsigned axis, int at)
{
    ...
    switch (decoded.type)
    {
        ...
        case slice_position_type::DOUBLE:
            return interpolated_slice(
                img, axis, decoded.slice0, decoded.slice1, decoded.offset);
    }
}
...
extract_slice_descriptions(
    std::vector<image_slice<N, Pixel, SP>> const& slices)
{
    size_t n_slices = slices.size();
    ...
    double position)
{
    ...
    for (size_t i = 1; i < slices.size(); ++i)
    {
        ...
        {
            return (position - slices[i - 1].position
                    < slices[i].position - position)
                       ? slices[i - 1]
                       : slices[i];
        }
    }
...
template<unsigned N>
string
sliced_image_units(std::vector<image_slice<N, variant, shared>> const& slices)
{
    if (slices.empty())
        return string();
