#ifndef ASTROID_IO_TEXT_PARSER_HPP
#define ASTROID_IO_TEXT_PARSER_HPP
...
initialize(
    ...
    char const* text,
    size_t text_size,
    ownership_holder const& ownership = ownership_holder());
...
void
initialize_parser_with_blob(
    text_parser& p, string const& label, cradle::blob const& b);
...
#ifdef _MSC_VER
void
read(text_parser& p, int& x);
void
read(text_parser& p, unsigned& x);
#endif
void
...
static inline string
read_quoted_string(text_parser& p)
{
    string x;
    read_quoted_string(p, x);
    return x;
}
...
static inline string
read_rest_of_line(text_parser& p)
{
    string x;
    read_rest_of_line(p, x);
    return x;
}
...
struct parse_error : exception
{
    parse_error(
        ...
        : astroid::exception(
            "parse error: " + label + ":" + to_string(line_number) + ":"
            + to_string(column_number) + ": " + message),
