#ifndef ASTROID_IO_RAW_MEMORY_IO_HPP
#define ASTROID_IO_RAW_MEMORY_IO_HPP
...
void
raw_read(raw_memory_reader& r, void* dst, size_t size);

template<class Integer>
Integer
read_int(raw_memory_reader& r)
{
    Integer i;
...
string
read_string(raw_memory_reader& r, size_t length);

template<class LengthType>
string
read_string(raw_memory_reader& r)
{
    auto length = read_int<LengthType>(r);
    return read_string(r, length);
}

void static inline advance(raw_memory_reader& r, size_t size)
{
    r.buffer += size;
    r.size -= size;
}
...
void
raw_write(raw_memory_writer& w, void const* src, size_t size);

template<class Integer>
void
write_int(raw_memory_writer& w, Integer i)
{
    swap_on_little_endian(&i);
...
void
write_float(raw_memory_writer& w, float f);

// Write the characters in a string, but not its length.
void
write_string_contents(raw_memory_writer& w, string const& s);

template<class LengthType>
void
write_string(raw_memory_writer& w, string const& s)
{
